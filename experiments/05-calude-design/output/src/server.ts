...
nextApp.prepare().then(() => {
  ...
  function titleSimilarity(a: string, b: string): number {
    ...
    const intersect = [...aW].filter(w => bW.has(w)).length;
    const union = new Set([...aW, ...bW]).size;
    return union > 0 ? intersect / union : 0;
  }

  // Merge activities (facilitator) — creates a new card, keeps originals
  app.post('/api/sessions/:id/activities/merge', (req, res) => {
    ...
    const sources = sourceIds.map(sid => session.activities.get(sid)).filter(Boolean) as import('./store').Activity[];
    ...
    const newActivity: import('./store').Activity = {
      ...
      tpo: (tpo ?? first.tpo) as import('./store').TimePerOccurrence,
      freq: (freq ?? first.freq) as import('./store').Frequency,
      energy: (energy ?? first.energy) as import('./store').Energy,
      teamAuto: 'unclassified', flagged: false, discussionNote: '',
      createdAt: new Date(), editHistory: [],
