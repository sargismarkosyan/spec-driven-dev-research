...
type Activity = {
  ...
  createdAt: string;
  // Merge tracking — source activities
  isMergedSource?: boolean;
  mergedIntoId?: string;
  // Merge tracking — merged result activities
...
export default function FacilitatorView({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    ...
    socket.on('activity:merged', ({ newActivity, updatedSources }: { newActivity: Activity; updatedSources: Activity[] }) => {
      setActivities(prev => {
        ...
        // Replace source activities with updated versions (isMergedSource: true), add new merged activity
        const without = prev.filter(x => !sourceIds.has(x.id));
        return [...without, ...updatedSources, newActivity];
      });
    });
  ...
  const handleMerge = useCallback((sourceIds: string[], mergedTitle: string, tpo: TimePerOccurrence, freq: Frequency, energy: Energy) => {
    socket.emit('activity:merge', { sessionId: id, sourceIds, title: mergedTitle, tpo, freq, energy, token });
    setMergeSource(null);
  }, [id, token]);
  ...
  // Facilitator views filter out isMergedSource activities
  ...
  return (
    <div className="wa-layout">
      ...
      <div style={{ flex: 1, overflow: 'hidden', position: 'relative' }}>
        {view === 'live'    && <LiveView activities={activeActivities} participants={participants} perPerson={perPerson} onEdit={openEdit} onRemove={handleRemove} onMerge={setMergeSource} />}
        {view === 'matrix'  && <MatrixView activities={activeActivities} selected={selectedActivity} onSelect={setSelectedActivity} onEdit={openEdit} onRemove={handleRemove} onFlag={handleFlag} onClassify={handleClassify} onMerge={setMergeSource} />}
        {view === 'grouped' && <GroupedView activities={activeActivities} onEdit={openEdit} onRemove={handleRemove} onMerge={setMergeSource} />}
        {view === 'discuss' && (
          <DiscussView
...
function computeCandidateSimilarity(source: Activity, candidate: Activity): number {
  const titleSim = jaccardSim(tokenize(source.title), tokenize(candidate.title));
...
function MergeModal({ source, allActivities, onMerge, onClose }: {
  source: Activity;
  allActivities: Activity[];
  onMerge: (sourceIds: string[], title: string, tpo: TimePerOccurrence, freq: Frequency, energy: Energy) => void;
  onClose: () => void;
}) {
  const [selected, setSelected] = useState<Set<string>>(new Set([source.id]));
  const [search, setSearch]     = useState('');
  const [mergeTitle, setMergeTitle] = useState(source.title);
  const [mergeTpo, setMergeTpo]     = useState<TimePerOccurrence>(source.tpo);
  const [mergeFreq, setMergeFreq]   = useState<Frequency>(source.freq);
  const [mergeEnergy, setMergeEnergy] = useState<Energy>(source.energy);

  const searchRef = useRef<HTMLInputElement>(null);
  useEffect(() => { searchRef.current?.focus(); }, []);

  // Compute candidates with similarity scores (client-side, no network call)
  const candidates = allActivities
    .filter(a => a.id !== source.id && !a.isMergedSource)
    .map(a => ({ ...a, similarity: computeCandidateSimilarity(source, a) }))
  ...
  // Selected activities (source + chosen candidates)
  const selectedActivities = allActivities.filter(a => selected.has(a.id));
  const totalHrsPerWk = selectedActivities.reduce((sum, a) => {
    const h = { '<30m': 0.5, '30m-2h': 1.25, 'half-day': 4, 'day+': 8 }[a.tpo] ?? 1;
    const f = { daily: 5, weekly: 1, monthly: 0.23, quarterly: 0.077, adhoc: 0.3 }[a.freq] ?? 1;
    return sum + h * f;
  }, 0);
  ...
  const simBadgeTone = (score: number) =>
    score > 0.60 ? 'is-rust' : score > 0.30 ? 'is-amber' : '';
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 60 }}>
      ...
      <div style={{
        ...
        {/* Header */}
        <div style={{ padding: '16px 22px', borderBottom: '1px solid var(--border-soft)', display: 'flex', alignItems: 'flex-start', gap: 14 }}>
          <div style={{ flex: 1 }}>
          ...
          <button onClick={onClose} style={{ background: 'transparent', border: 0, fontSize: 20, color: 'var(--muted)', cursor: 'pointer', padding: 4, flexShrink: 0 }}>×</button>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', flex: 1, overflow: 'hidden' }}>
          ...
          <div style={{ padding: '18px 20px', borderRight: '1px solid var(--rule)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            <div className="wa-eyebrow" style={{ marginBottom: 12 }}>
              Select cards to merge · <strong>{selected.size}</strong> selected
            </div>
            ...
            <div style={{
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 13, fontWeight: 500, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{source.title}</div>
              ...
              <span className="wa-chip is-ghost" style={{ fontSize: 10, flexShrink: 0 }}>starting card</span>
            </div>
            ...
            <div style={{ position: 'relative', marginBottom: 8 }}>
              ...
              <input
                ref={searchRef}
                className="wa-input"
                style={{ paddingLeft: 28, paddingRight: search ? 28 : 10, fontSize: 13 }}
              ...
              {search && (
                <button onClick={() => setSearch('')} style={{
                  ...
                  background: 'transparent', border: 0, color: 'var(--muted)', cursor: 'pointer', fontSize: 14, padding: 0,
                }}>×</button>
              )}
            </div>

            {/* Candidate list */}
            <div style={{ flex: 1, overflowY: 'auto', display: 'grid', gap: 6, paddingRight: 2 }}>
              {filtered.length === 0 ? (
                <div style={{ fontSize: 12, color: 'var(--muted-2)', fontStyle: 'italic', padding: '8px 0' }}>
                  No activities match &lsquo;{search}&rsquo;.
                </div>
              ) : (
                filtered.map(a => {
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        background: on ? 'var(--paper-deep)' : '#fff',
                        border: `1px solid ${on ? 'var(--ink)' : 'var(--border-soft)'}`,
                        display: 'flex', alignItems: 'center', gap: 10,
                      }}
                    >
                      <span style={{
                      ...
                      {a.similarity > 0.15 && (
                        <span className={`wa-chip ${simBadgeTone(a.similarity)}`} style={{ fontSize: 10, flexShrink: 0 }}>
          ...
          <div style={{ padding: '18px 20px', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
                ...
                {selectedActivities.map((a, i) => (
                  <span
                    ...
                    style={{
                      ...
                      position: 'relative', zIndex: selectedActivities.length - i,
                    }}
              ...
              <span className="wa-chip is-sage" style={{ fontSize: 10, flexShrink: 0, whiteSpace: 'nowrap' }}>
                ⇄ {selected.size} cards · ~{totalHrsPerWk.toFixed(1)} h/wk
              </span>
            </div>
            ...
            <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 14 }}>
                  ...
                  {([['<30m','<30 min'],['30m-2h','30m–2h'],['half-day','½ day'],['day+','1+ day']] as [TimePerOccurrence, string][]).map(([v, lbl]) => {
                    ...
                    return (
                      <button key={v} onClick={() => setMergeTpo(v)} style={{
                        padding: '8px 6px', fontSize: 11.5, fontWeight: on ? 600 : 400, textAlign: 'center',
                  ...
                  {([['daily','Daily'],['weekly','Weekly'],['monthly','Monthly'],['quarterly','Qtrly'],['adhoc','Ad hoc']] as [Frequency, string][]).map(([v, lbl]) => {
                    ...
                    return (
                      <button key={v} onClick={() => setMergeFreq(v)} style={{
                        padding: '8px 4px', fontSize: 11, fontWeight: on ? 600 : 400, textAlign: 'center',
                        background: on ? 'var(--ink)' : '#fff', color: on ? 'var(--cream)' : 'var(--ink)',
                        border: `1px solid ${on ? 'var(--ink)' : 'var(--rule)'}`, borderRadius: 4, cursor: 'pointer',
                      }}>{lbl}</button>
                    );
              ...
              {/* Footer note */}
              <div style={{ marginTop: 4, fontSize: 11.5, color: 'var(--muted)', fontStyle: 'italic', lineHeight: 1.5 }}>
        ...
        <div style={{
          ...
          <span style={{ fontSize: 12, color: canMerge ? 'var(--ink-2)' : 'var(--muted)', fontStyle: canMerge ? 'normal' : 'italic', flex: 1 }}>
            {canMerge
