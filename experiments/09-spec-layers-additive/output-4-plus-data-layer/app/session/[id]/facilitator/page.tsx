...
export default function FacilitatorView({ params }: { params: { id: string } }) {
  ...
  const openMerge = useCallback((a: Activity) => {
    setMergeSourceActivity(a);
  }, []);

  const handleMerge = useCallback((sourceIds: string[], title: string, tpo: TimePerOccurrence, freq: Frequency, energy: Energy) => {
    socket.emit('activity:merge', { sessionId: id, sourceIds, title, tpo, freq, energy, token });
  ...
  // Per-person counts: count non-merged-source activities
  ...
  return (
    <div className="wa-layout">
      ...
      <div style={{ flex: 1, overflow: 'hidden', position: 'relative' }}>
        {view === 'live'    && <LiveView activities={activities} participants={participants} perPerson={perPerson} onEdit={openEdit} onRemove={handleRemove} onMerge={openMerge} />}
...
// FacActions with a merge button between edit and remove
function FacActionsWithMerge({
  size = 'md',
  onEdit,
  onMerge,
  onRemove,
...
// Stacked avatar row (each offset 8px left, 2px white border)
function AvatarStack({ initials, colors, size = 'sm' }: {
  initials: string[];
  colors: string[];
  size?: 'sm' | 'md';
}) {
  ...
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', position: 'relative', height: dim }}>
      {initials.map((ini, i) => (
        <span
          key={i}
          style={{
            ...
            background: colors[i] ?? '#999', color: '#fff',
            fontSize: size === 'sm' ? 8 : 10, fontWeight: 600,
            fontFamily: 'var(--font-mono)',
            border: '2px solid #fff',
            marginLeft: i === 0 ? 0 : -8,
            position: 'relative', zIndex: initials.length - i,
          }}
        >
          {ini}
        </span>
...
// Jaccard similarity for merge candidate scoring
...
function combinedSimilarity(source: Activity, candidate: Activity): number {
  const titleSim = jaccardSimilarity(source.title, candidate.title);
...
function LiveCard({ a, onEdit, onRemove, onMerge, allActivities }: {
  ...
  const isMergedResult = !!(a.mergedFromIds && a.mergedFromIds.length > 0);
  const sourceActivities = isMergedResult
    ? allActivities.filter(x => a.mergedFromIds!.includes(x.id))
    : [];

  return (
    <div className="wa-activity">
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
        {isMergedResult && a.reportedByInitials && a.reportedByColors ? (
          <AvatarStack initials={a.reportedByInitials} colors={a.reportedByColors} />
        ) : (
          <span className="wa-avatar is-sm" style={{ background: a.participantColor, color: '#fff' }}>{a.participantInitials}</span>
        )}
        <span style={{ fontSize: 12, color: 'var(--muted)' }}>
          {isMergedResult && a.reportedBy ? a.reportedBy.map(n => n.split(' ')[0]).join(' + ') : a.participantName}
        </span>
        {isMergedResult && (
          <span className="wa-chip is-sage" style={{ fontSize: 10, padding: '2px 6px' }}>
            ⇄ {a.mergedFromIds!.length} cards
          </span>
        )}
        <span style={{ marginLeft: 'auto' }}>
          <FacActionsWithMerge size="sm" onEdit={() => onEdit(a)} onRemove={() => onRemove(a.id)} onMerge={() => onMerge(a)} />
        </span>
      </div>
      ...
      {isMergedResult && sourceActivities.length > 0 && (
        <div style={{ marginTop: 8 }}>
          ...
          >
            {sourcesOpen ? '▲' : '▼'} {sourceActivities.length} original cards
          </button>
          {sourcesOpen && (
            <div style={{ marginTop: 6, display: 'grid', gap: 4 }}>
              {sourceActivities.map(src => (
                <div key={src.id} style={{
                  opacity: 0.5, padding: '7px 10px',
                  background: 'var(--paper-deep)', borderRadius: 4,
                  border: '1px solid var(--border-soft)',
                }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                    <span className="wa-avatar is-sm" style={{ background: src.participantColor, color: '#fff' }}>{src.participantInitials}</span>
                    <span style={{ fontSize: 11, color: 'var(--muted)' }}>{src.participantName.split(' ')[0]}</span>
                    <span className="wa-chip" style={{ fontSize: 9, padding: '1px 5px', marginLeft: 'auto' }}>merged source</span>
                  </div>
                  <div style={{ fontSize: 12.5, fontWeight: 500, color: 'var(--ink-2)' }}>{src.title}</div>
                </div>
              ))}
            </div>
          )}
...
function LiveView({ activities, participants, perPerson, onEdit, onRemove, onMerge }: {
  activities: Activity[];
  ...
  // Show recent non-merged-source activities first, then all sorted by time
  const visibleActivities = activities.filter(a => !a.isMergedSource);
  const recent = [...visibleActivities].sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()).slice(0, 10);

  // Theme aggregation (using visible non-source activities)
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '260px 1fr 1fr', height: '100%' }}>
      ...
      <aside style={{ padding: '24px 18px', borderRight: '1px solid var(--rule)', background: 'var(--paper)', overflowY: 'auto' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13 }}>
          ...
          <span className="wa-mono" style={{ fontWeight: 600 }}>{visibleActivities.length}</span>
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, marginTop: 4 }}>
          <span style={{ color: 'var(--muted)' }}>Per-engineer avg</span>
          <span className="wa-mono" style={{ fontWeight: 600 }}>
            {participants.length > 0 ? (visibleActivities.length / participants.length).toFixed(1) : '—'}
          </span>
        </div>
      </aside>
      ...
      <main style={{ padding: '24px 28px', overflowY: 'auto', background: 'var(--cream)', borderRight: '1px solid var(--rule)' }}>
        ...
        {recent.length === 0 ? (
          ...
          <div style={{ display: 'grid', gap: 8 }}>
            {recent.map(a => (
              <LiveCard key={a.id} a={a} onEdit={onEdit} onRemove={onRemove} onMerge={onMerge} allActivities={activities} />
            ))}
          </div>
        )}
      </main>
        ...
        {sel ? (
          <>
            ...
            <div style={{ marginTop: 10, display: 'flex', gap: 6, justifyContent: 'center' }}>
              <FacActionsWithMerge onEdit={() => onEdit(sel)} onRemove={() => onRemove(sel.id)} onMerge={() => onMerge(sel)} />
            </div>
            {sel.discussionNote && (
          ...
          <>
            <div style={{ padding: '18px 20px 14px', borderBottom: '1px solid var(--border-soft)' }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, alignItems: 'center' }}>
                ...
                <span style={{ marginLeft: 'auto' }}>
                  <FacActionsWithMerge size="sm" onEdit={() => onEdit(foc)} onRemove={() => onRemove(foc.id)} onMerge={() => onMerge(foc)} />
                </span>
              </div>
            </div>
...
function MergeModal({
  ...
  allActivities: Activity[];
  onMerge: (sourceIds: string[], title: string, tpo: TimePerOccurrence, freq: Frequency, energy: Energy) => void;
  onClose: () => void;
}) {
  const [selected, setSelected] = useState<Set<string>>(new Set([sourceActivity.id]));
  const [search, setSearch] = useState('');
  const [mergeTitle, setMergeTitle] = useState(sourceActivity.title);
  const [mergeTpo, setMergeTpo] = useState<TimePerOccurrence>(sourceActivity.tpo);
  const [mergeFreq, setMergeFreq] = useState<Frequency>(sourceActivity.freq);
  const [mergeEnergy, setMergeEnergy] = useState<Energy>(sourceActivity.energy);
  ...
  // Candidates: all non-isMergedSource activities except the source, sorted by similarity desc
  const candidates = useMemo(() => {
    const raw = allActivities.filter(a => !a.isMergedSource && a.id !== sourceActivity.id);
    return raw
      .map(a => ({ activity: a, score: combinedSimilarity(sourceActivity, a) }))
      .sort((a, b) => b.score - a.score);
  }, [allActivities, sourceActivity]);

  // Filtered candidates by search
  const filteredCandidates = useMemo(() => {
    if (!search) return candidates;
    const q = search.toLowerCase();
    return candidates.filter(({ activity: a }) =>
  ...
  // All selected activities (source + toggled candidates)
  const selectedActivities = useMemo(() => {
    return allActivities.filter(a => selected.has(a.id));
  }, [allActivities, selected]);

  const totalHrs = selectedActivities.reduce((sum, a) => sum + calcEffort(a.tpo, a.freq).hrs, 0);

  const handleSubmit = () => {
    if (selected.size < 2) return;
    onMerge(Array.from(selected), mergeTitle, mergeTpo, mergeFreq, mergeEnergy);
  };

  const similarityBadgeTone = (score: number) => {
    if (score > 0.60) return 'is-rust';
    if (score > 0.30) return 'is-amber';
    return '';
  };
      ...
      <div style={{
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', flex: 1, overflow: 'hidden' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', borderRight: '1px solid var(--border-soft)', overflow: 'hidden' }}>
            <div style={{ padding: '14px 18px', borderBottom: '1px solid var(--border-soft)', flexShrink: 0 }}>
              <div className="wa-eyebrow" style={{ marginBottom: 2 }}>Select cards to merge · {selected.size} selected</div>
            </div>
            ...
            <div style={{ overflowY: 'auto', flex: 1, padding: '12px 16px', display: 'flex', flexDirection: 'column', gap: 6 }}>
              ...
              <div style={{
                ...
                padding: '10px 12px', background: 'var(--ink)', borderRadius: 5,
                border: '1px solid var(--ink)',
              }}>
                {/* Filled dark checkbox */}
                <span style={{
                ...
                <span
                  ...
                  style={{ background: sourceActivity.participantColor, color: '#fff', flexShrink: 0 }}
                >
                  {sourceActivity.participantInitials}
                </span>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 12.5, fontWeight: 500, color: 'var(--cream)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                    {sourceActivity.title}
                  </div>
                  <div className="wa-mono" style={{ fontSize: 10, color: 'rgba(250,247,242,0.65)', marginTop: 2 }}>
                    {sourceActivity.participantName.split(' ')[0]} · {TPO_SHORT[sourceActivity.tpo]} · {FREQ_SHORT[sourceActivity.freq]}
                  </div>
                </div>
                <span style={{
                  fontSize: 10, padding: '2px 7px', borderRadius: 10,
                  border: '1px solid rgba(250,247,242,0.4)', color: 'rgba(250,247,242,0.7)',
                  fontFamily: 'var(--font-mono)', whiteSpace: 'nowrap', flexShrink: 0,
                }}>starting card</span>
              </div>

              {/* Search input */}
              <div style={{ position: 'relative', marginTop: 2, marginBottom: 4 }}>
                <span style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)', fontSize: 13, color: 'var(--muted)', pointerEvents: 'none' }}>⌕</span>
                <input
                  ref={searchInputRef}
                  className="wa-input"
                  style={{ paddingLeft: 28, paddingRight: search ? 28 : 10, fontSize: 12.5 }}
                ...
                {search && (
                  <button
                    onClick={() => setSearch('')}
                    style={{ position: 'absolute', right: 8, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', fontSize: 13, color: 'var(--muted)', padding: 2, lineHeight: 1 }}
                  >×</button>
                )}
              </div>
              ...
              {filteredCandidates.length === 0 && search ? (
                <div style={{ fontSize: 12, color: 'var(--muted-2)', fontStyle: 'italic', padding: '8px 4px' }}>
                  No activities match '{search}'.
                </div>
              ) : (
                filteredCandidates.map(({ activity: a, score }) => {
                  ...
                  return (
                    ...
                    >
                      <input
                        type="checkbox"
                        checked={isChecked}
                        onChange={() => toggleCandidate(a.id)}
                        style={{ width: 15, height: 15, flexShrink: 0, accentColor: 'var(--ink)', cursor: 'pointer' }}
                      />
                      <span
                        className="wa-avatar is-sm"
                        style={{ background: a.participantColor, color: '#fff', flexShrink: 0 }}
                      >
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div className="wa-mono" style={{ fontSize: 10, color: 'var(--muted)', marginTop: 2 }}>
                          {a.participantName.split(' ')[0]} · {TPO_SHORT[a.tpo]} · {FREQ_SHORT[a.freq]}
                        </div>
                      </div>
                      {score > 0.15 && (
                        <span className={`wa-chip ${similarityBadgeTone(score)}`} style={{ fontSize: 10, padding: '2px 6px', flexShrink: 0 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            <div style={{ padding: '14px 18px', borderBottom: '1px solid var(--border-soft)', flexShrink: 0 }}>
              <div className="wa-eyebrow">Merged card — edit before creating</div>
            </div>

            <div style={{ overflowY: 'auto', flex: 1, padding: '16px 18px', display: 'flex', flexDirection: 'column', gap: 14 }}>
              {/* Stacked avatars + author line + chip */}
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
                <AvatarStack
                  initials={selectedActivities.map(a => a.participantInitials)}
                  colors={selectedActivities.map(a => a.participantColor)}
                  size="md"
                />
                <span style={{ fontSize: 12, color: 'var(--muted)' }}>
                  {selectedActivities.map(a => a.participantName.split(' ')[0]).join(' + ')}
                </span>
                <span className="wa-chip is-sage" style={{ fontSize: 10, padding: '2px 7px', marginLeft: 'auto', whiteSpace: 'nowrap' }}>
              ...
              {/* Time per occurrence — 4-button grid */}
              <div>
                <label className="wa-label">Time per occurrence</label>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 5 }}>
                  {([['<30m', '<30m'], ['30m-2h', '30m–2h'], ['half-day', '½ day'], ['day+', '1+ day']] as [TimePerOccurrence, string][]).map(([v, lbl]) => {
                    const on = v === mergeTpo;
                    return (
                      <button key={v} onClick={() => setMergeTpo(v)} style={{
                        padding: '8px 6px', background: on ? 'var(--ink)' : '#fff',
                        color: on ? 'var(--cream)' : 'var(--ink-2)',
                        border: '1px solid ' + (on ? 'var(--ink)' : 'var(--rule)'),
                        borderRadius: 4, cursor: 'pointer', fontSize: 12, fontWeight: on ? 500 : 400,
                      }}>{lbl}</button>
                    );
                  })}
                </div>
              ...
              <div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 5 }}>
                  {([['daily', 'Daily'], ['weekly', 'Weekly'], ['monthly', 'Monthly'], ['quarterly', 'Qtrly'], ['adhoc', 'Ad hoc']] as [Frequency, string][]).map(([v, lbl]) => {
                    const on = v === mergeFreq;
                    return (
                      <button key={v} onClick={() => setMergeFreq(v)} style={{
                        padding: '8px 4px', background: on ? 'var(--ink)' : '#fff',
                        color: on ? 'var(--cream)' : 'var(--ink-2)',
                        border: '1px solid ' + (on ? 'var(--ink)' : 'var(--rule)'),
                        borderRadius: 4, cursor: 'pointer', fontSize: 11, fontWeight: on ? 500 : 400,
                      }}>{lbl}</button>
                    );
                  })}
                </div>
              ...
              {/* Energy — 4-button grid in 3-column layout */}
              <div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 5 }}>
                  {([['energizing', 'Energizing'], ['fine', 'Fine'], ['tedious', 'Tedious'], ['draining', 'Draining']] as [Energy, string][]).map(([v, lbl]) => {
                    const on = v === mergeEnergy;
                    return (
                      <button key={v} onClick={() => setMergeEnergy(v)} style={{
                        padding: '8px 6px', background: on ? 'var(--ink)' : '#fff',
                        color: on ? 'var(--cream)' : 'var(--ink-2)',
                        border: '1px solid ' + (on ? 'var(--ink)' : 'var(--rule)'),
                        borderRadius: 4, cursor: 'pointer', fontSize: 12, fontWeight: on ? 500 : 400,
              ...
              {/* Footer note */}
              <div style={{ fontSize: 11.5, color: 'var(--muted)', fontStyle: 'italic', lineHeight: 1.5, marginTop: 4 }}>
        ...
        <div style={{ padding: '14px 22px', borderTop: '1px solid var(--border-soft)', display: 'flex', alignItems: 'center', gap: 12 }}>
          <span style={{ fontSize: 12, color: selected.size >= 2 ? 'var(--ink-2)' : 'var(--muted)', flex: 1 }}>
            {selected.size >= 2
          ...
          <button
            ...
            style={{ padding: '8px 16px', fontSize: 13, opacity: selected.size < 2 ? 0.45 : 1, cursor: selected.size < 2 ? 'not-allowed' : 'pointer' }}
