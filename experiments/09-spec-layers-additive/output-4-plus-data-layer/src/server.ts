...
nextApp.prepare().then(() => {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('activity:merge', (data: {
      ...
      const sources = data.sourceIds.map(id => session.activities.get(id)).filter(Boolean) as import('./store').Activity[];
      if (sources.length !== data.sourceIds.length) return;

      const first = sources[0];

      // Aggregate reporters from all sources
      const reportedBy: string[] = [];
      const reportedByInitials: string[] = [];
      const reportedByColors: string[] = [];
      for (const src of sources) {
        if (!reportedBy.includes(src.participantName)) {
      ...
      // Create merged result activity
      const mergedId = crypto.randomUUID().slice(0, 8);
      const mergedActivity: import('./store').Activity = {
        id: mergedId,
        sessionId: data.sessionId,
      ...
      io.to(data.sessionId).emit('activity:added', mergedActivity);
    });
