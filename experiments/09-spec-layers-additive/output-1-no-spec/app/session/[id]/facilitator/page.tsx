...
export default function FacilitatorView({ params }: { params: { id: string } }) {
  ...
  const handleMerge = useCallback((primaryId: string, secondaryIds: string[], title: string) => {
    socket.emit('activity:merge', { sessionId: id, primaryId, secondaryIds, title, token });
    setMergeTarget(null);
  }, [id, token]);
  ...
  return (
    <div className="wa-layout">
      ...
      <div style={{ flex: 1, overflow: 'hidden', position: 'relative' }}>
        {view === 'live'    && <LiveView activities={activities} participants={participants} perPerson={perPerson} onEdit={openEdit} onRemove={handleRemove} onMerge={setMergeTarget} />}
        {view === 'matrix'  && <MatrixView activities={activeActivities} selected={selectedActivity} onSelect={setSelectedActivity} onEdit={openEdit} onRemove={handleRemove} onFlag={handleFlag} onClassify={handleClassify} onMerge={setMergeTarget} />}
        {view === 'grouped' && <GroupedView activities={activeActivities} onEdit={openEdit} onRemove={handleRemove} onMerge={setMergeTarget} />}
        {view === 'discuss' && (
          <DiscussView
      ...
      {mergeTarget && (
        <MergeModal
          primary={mergeTarget}
          candidates={activities.filter(a => a.id !== mergeTarget.id)}
          onMerge={handleMerge}
    ...
    <div className="wa-activity">
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
        ...
        <span style={{ marginLeft: 'auto', display: 'inline-flex', alignItems: 'center', gap: 4 }}>
          <button
            onClick={() => onMerge(a)}
            title="Merge similar activities into this one"
            style={{
      ...
      {a.mergedFrom && a.mergedFrom.length > 0 && (
        <div style={{ marginTop: 6, fontSize: 10.5, color: 'var(--muted)', fontFamily: 'var(--font-mono)' }}>
          ↳ merged: {a.mergedFrom.join(' · ')}
        </div>
      )}
    </div>
  );
      ...
      <aside style={{ padding: '24px 22px', borderLeft: '1px solid var(--rule)', background: 'var(--paper)', overflowY: 'auto' }}>
        {sel ? (
          <>
            ...
            {sel.mergedFrom && sel.mergedFrom.length > 0 && (
              <div style={{ marginTop: 8, fontSize: 10.5, color: 'var(--muted)', fontFamily: 'var(--font-mono)', lineHeight: 1.5 }}>
                ↳ merged: {sel.mergedFrom.join(' · ')}
              </div>
            )}
            {sel.discussionNote && (
...
function GroupedCard({ a, rank, onEdit, onRemove, onMerge }: {
  ...
  return (
    <div className={`wa-activity${a.flagged ? ' wa-flagged' : ''}`} style={{ padding: '10px 12px', position: 'relative' }}>
      ...
      {a.mergedFrom && a.mergedFrom.length > 0 && (
        <div style={{ marginTop: 5, fontSize: 10, color: 'var(--muted)', fontFamily: 'var(--font-mono)', lineHeight: 1.4 }}>
          ↳ merged: {a.mergedFrom.join(' · ')}
        </div>
      )}
    </div>
...
function MergeModal({
  ...
  primary: Activity;
  candidates: Activity[];
  onMerge: (primaryId: string, secondaryIds: string[], title: string) => void;
  onClose: () => void;
}) {
  const [selected, setSelected] = useState<Set<string>>(new Set());
  const [mergedTitle, setMergedTitle] = useState(primary.title);
  ...
  const selectedActivities = candidates.filter(a => selected.has(a.id));

  const handleConfirm = () => {
    if (selected.size === 0) return;
    onMerge(primary.id, Array.from(selected), mergedTitle);
  };
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 50 }}>
      ...
      <div style={{
        position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)',
        width: 740, maxHeight: '90dvh', background: 'var(--cream)',
        borderRadius: 8, boxShadow: '0 24px 80px rgba(0,0,0,0.32)',
        border: '1px solid var(--rule)', overflow: 'hidden', display: 'flex', flexDirection: 'column',
      }}>
        {/* Header */}
        <div style={{ padding: '16px 22px', borderBottom: '1px solid var(--border-soft)', display: 'flex', alignItems: 'center', gap: 14 }}>
          <div style={{ flex: 1 }}>
            <Eyebrow color="var(--rust)">↳ Facilitator action · merge similar activities</Eyebrow>
            <h2 className="wa-display" style={{ fontSize: 20, margin: '4px 0 0', fontWeight: 500 }}>
              Merge into this activity
            </h2>
          </div>
          <button onClick={onClose} style={{ background: 'transparent', border: 0, fontSize: 20, color: 'var(--muted)', cursor: 'pointer', padding: 4 }}>×</button>
        </div>

        {/* Primary activity preview */}
        <div style={{ padding: '12px 22px', borderBottom: '1px solid var(--border-soft)', background: 'var(--rust-bg)', display: 'flex', alignItems: 'center', gap: 12 }}>
          <span className="wa-avatar is-sm" style={{ background: primary.participantColor, color: '#fff' }}>{primary.participantInitials}</span>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div className="wa-mono" style={{ fontSize: 10, color: 'var(--muted-2)', letterSpacing: '0.06em', marginBottom: 2 }}>
              ↳ PRIMARY (keeps this card, absorbs others)
            </div>
            <div style={{ fontSize: 13, fontWeight: 500 }}>{primary.title}</div>
            <div style={{ fontSize: 11, color: 'var(--muted)' }}>{primary.participantName}</div>
          </div>
          <EffortPill tpo={primary.tpo} freq={primary.freq} size="sm" />
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', flex: 1, overflow: 'hidden' }}>
          ...
          <div style={{ borderRight: '1px solid var(--rule)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--border-soft)' }}>
              <div className="wa-eyebrow" style={{ marginBottom: 8 }}>↳ Select activities to merge in</div>
              <input
                className="wa-input"
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '10px 12px', display: 'grid', gap: 6 }}>
              {filteredCandidates.length === 0 && (
                <div style={{ fontSize: 12, color: 'var(--muted-2)', fontStyle: 'italic', padding: '12px 4px' }}>No matching activities.</div>
              )}
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      background: on ? 'var(--rust-bg)' : '#fff',
                      border: `1px solid ${on ? '#e8c8b8' : 'var(--border-soft)'}`,
                      borderRadius: 5, cursor: 'pointer', textAlign: 'left',
                    }}
                  >
                    <span style={{
                      width: 16, height: 16, borderRadius: 3, flexShrink: 0, marginTop: 1,
                      background: on ? 'var(--rust)' : '#fff',
                      border: `1.5px solid ${on ? 'var(--rust)' : 'var(--rule)'}`,
                      display: 'flex', alignItems: 'center', justifyContent: 'center',
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 12.5, fontWeight: 500, lineHeight: 1.3, marginBottom: 3 }}>{a.title}</div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                        <span className="wa-avatar is-sm" style={{ background: a.participantColor, color: '#fff', width: 14, height: 14, fontSize: 8 }}>{a.participantInitials}</span>
                        <span style={{ fontSize: 10.5, color: 'var(--muted)' }}>{a.participantName.split(' ')[0]}</span>
                        <EffortPill tpo={a.tpo} freq={a.freq} size="sm" />
                        <span className={`wa-chip ${ENERGY_TONE[a.energy]}`} style={{ padding: '1px 4px', fontSize: 9 }}>{ENERGY_SHORT[a.energy]}</span>
                      </div>
                    </div>
                  </button>
                );
          ...
          {/* Right — merged result preview + title edit */}
          <div style={{ display: 'flex', flexDirection: 'column', padding: '16px 18px', overflow: 'hidden' }}>
            <div className="wa-eyebrow" style={{ marginBottom: 10 }}>↳ Merged result title</div>
            <input
              className="wa-input"
            ...
            />
            <div className="wa-mono" style={{ fontSize: 10, color: 'var(--muted-2)', marginBottom: 14, letterSpacing: '0.03em' }}>
              ↳ The selected activities below will be removed and absorbed into the primary.
            </div>

            {selected.size === 0 ? (
              <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--muted-2)', fontSize: 12, fontStyle: 'italic', textAlign: 'center', padding: 16 }}>
                Select one or more activities on the left to preview the merge.
              </div>
            ) : (
              <div style={{ flex: 1, overflowY: 'auto' }}>
                <div className="wa-eyebrow" style={{ marginBottom: 8 }}>
                  ↳ Will be absorbed ({selected.size})
                </div>
                  ...
                  {selectedActivities.map(a => (
                    <div key={a.id} style={{
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ fontSize: 10, color: 'var(--muted)' }}>{a.participantName.split(' ')[0]}</div>
                      </div>
                      <button
                        onClick={() => toggle(a.id)}
                        title="Remove from merge"
                        style={{ background: 'transparent', border: 0, cursor: 'pointer', color: 'var(--muted-2)', fontSize: 14, padding: '0 2px' }}
                      >
                        ×
                      </button>
                    </div>
        ...
        <div style={{ padding: '14px 22px', borderTop: '1px solid var(--border-soft)', display: 'flex', gap: 10, alignItems: 'center' }}>
          <div style={{ flex: 1, fontSize: 12, color: 'var(--muted)' }}>
            {selected.size === 0
              ? 'Select at least one activity to merge.'
              : `Merge ${selected.size} activit${selected.size === 1 ? 'y' : 'ies'} into "${mergedTitle || primary.title}"`}
          </div>
          <button className="wa-btn is-ghost" style={{ padding: '8px 14px', fontSize: 13 }} onClick={onClose}>Cancel</button>
          <button
            className="wa-btn is-rust"
            style={{ padding: '8px 18px', fontSize: 13, opacity: selected.size === 0 ? 0.45 : 1 }}
            disabled={selected.size === 0}
            onClick={handleConfirm}
          >
            Merge {selected.size > 0 ? `(${selected.size + 1} → 1)` : ''}
          </button>
        </div>
      </div>
    </div>
