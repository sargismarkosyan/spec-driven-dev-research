...
nextApp.prepare().then(() => {
  ...
  // Merge activities (facilitator — takes one primary + N secondaries, deletes secondaries)
  app.post('/api/sessions/:id/activities/merge', (req, res) => {
    const session = sessions.get(req.params.id);
    if (!session) return res.status(404).json({ error: 'not found' });
    if (req.body.token !== session.facilitatorToken) return res.status(403).json({ error: 'forbidden' });

    const { primaryId, secondaryIds, title } = req.body as {
      primaryId: string;
      secondaryIds: string[];
      title?: string;
      token: string;
    ...
    const primary = session.activities.get(primaryId);
    if (!primary) return res.status(404).json({ error: 'primary activity not found' });

    const absorbed: { title: string; participantName: string }[] = [];
    for (const sid of secondaryIds) {
      const sec = session.activities.get(sid);
      if (!sec) continue;
      absorbed.push({ title: sec.title, participantName: sec.participantName });
      session.activities.delete(sid);
      io.to(req.params.id).emit('activity:deleted', { id: sid });
    }

    if (title !== undefined && title.trim()) primary.title = title.trim();
    primary.mergedFrom = [...(primary.mergedFrom ?? []), ...absorbed.map(a => a.title)];
    primary.mergedParticipants = [...new Set([
      ...(primary.mergedParticipants ?? []),
      ...absorbed.map(a => a.participantName),
    ])];
    primary.editedBy = session.facilitatorName;
    primary.editHistory.push({
      who: session.facilitatorName,
      what: `merged with: ${absorbed.map(a => `"${a.title}"`).join(', ')}`,
      at: new Date(),
    });

    io.to(req.params.id).emit('activity:updated', primary);
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('activity:merge', (data: {
      sessionId: string; primaryId: string; secondaryIds: string[]; title?: string; token: string;
    }) => {
      const session = sessions.get(data.sessionId);
      if (!session || data.token !== session.facilitatorToken) return;
      const primary = session.activities.get(data.primaryId);
      if (!primary) return;

      const absorbed: { title: string; participantName: string }[] = [];
      for (const sid of data.secondaryIds) {
        const sec = session.activities.get(sid);
        if (!sec) continue;
        absorbed.push({ title: sec.title, participantName: sec.participantName });
        session.activities.delete(sid);
        io.to(data.sessionId).emit('activity:deleted', { id: sid });
      }

      if (data.title !== undefined && data.title.trim()) primary.title = data.title.trim();
      primary.mergedFrom = [...(primary.mergedFrom ?? []), ...absorbed.map(a => a.title)];
      primary.mergedParticipants = [...new Set([
        ...(primary.mergedParticipants ?? []),
        ...absorbed.map(a => a.participantName),
      ...
      io.to(data.sessionId).emit('activity:updated', primary);
    });
