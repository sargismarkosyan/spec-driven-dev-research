...
function combinedScore(a: Activity, b: Activity): number {
...
type Candidate = { activity: Activity; score: number };
...
export default function FacilitatorView({ params }: { params: { id: string } }) {
  ...
  // Merge modal state
  const [mergeAnchor, setMergeAnchor]   = useState<Activity | null>(null);
  ...
  useEffect(() => {
    ...
    socket.on('activity:merged', ({ merged, sourceIds }: { merged: Activity; sourceIds: string[] }) => {
      setActivities(prev => {
        const updated = prev.map(x =>
          sourceIds.includes(x.id) ? { ...x, isMergedSource: true, mergedIntoId: merged.id } : x
        );
        // Add merged activity if not already present
        if (!updated.find(x => x.id === merged.id)) return [...updated, merged];
        return updated;
      });
    });
  ...
  const openMerge = useCallback((anchor: Activity) => {
    // Compute candidates client-side — no network call
    const pool = activities.filter(a => !a.isMergedSource && a.id !== anchor.id);
    const candidates = pool
      .map(a => ({ activity: a, score: combinedScore(anchor, a) }))
      .sort((a, b) => b.score - a.score);

    setMergeAnchor(anchor);
    setMergeCandidates(candidates);
    const preselected = new Set([anchor.id]);
    setMergeSelected(preselected);
    // Pre-fill title/tpo/freq/energy from anchor
    setMergeTitle(anchor.title);
    setMergeTpo(anchor.tpo);
    setMergeFreq(anchor.freq);
    setMergeEnergy(anchor.energy);
  }, [activities]);

  const handleMergeToggle = useCallback((activityId: string) => {
    setMergeSelected(prev => {
      const next = new Set(prev);
      // The anchor is always included and cannot be deselected
      if (mergeAnchor?.id === activityId) return prev;
  ...
  const handleMergeConfirm = useCallback(() => {
    if (!mergeAnchor || mergeSelected.size < 2) return;
  ...
  }, [id, token, mergeAnchor, mergeSelected, mergeTitle, mergeTpo, mergeFreq, mergeEnergy]);
  ...
  // Facilitator views exclude isMergedSource activities from lists/counts/trays
  ...
  return (
    <div className="wa-layout">
      ...
      <div className="wa-topbar">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          ...
          <div className="wa-eyebrow" style={{ fontSize: 10 }}>
            ...
            {activities.filter(a => a.isMergedSource).length > 0 ? ` · ${activities.filter(a => a.isMergedSource).length} merged` : ''}
      ...
      <div style={{ flex: 1, overflow: 'hidden', position: 'relative' }}>
        ...
        {view === 'matrix'  && <MatrixView activities={activeActivities} selected={selectedActivity} onSelect={setSelectedActivity} onEdit={openEdit} onRemove={handleRemove} onFlag={handleFlag} onClassify={handleClassify} />}
        {view === 'grouped' && <GroupedView activities={activeActivities} onEdit={openEdit} onRemove={handleRemove} />}
        {view === 'discuss' && (
          <DiscussView
...
function LiveCard({ a, onEdit, onRemove, onMerge, sources }: {
  a: Activity;
  onEdit: (a: Activity) => void;
  onRemove: (id: string) => void;
  onMerge?: (a: Activity) => void;
  sources?: Activity[];  // populated when this is a merged result card
}) {
  const [showSources, setShowSources] = useState(false);
  const isMergedResult = sources && sources.length > 0;

  return (
    <div className="wa-activity" style={isMergedResult ? { borderLeft: '3px solid var(--amber)', background: 'var(--amber-bg)' } : {}}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
        ...
        {isMergedResult && (
          <span className="wa-chip is-amber" style={{ fontSize: 10, padding: '1px 5px', fontWeight: 600 }}>
            ⊕ Merged · {sources.length} sources
          </span>
        )}
        <span style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 4 }}>
          {onMerge && !isMergedResult && (
            <button
              style={{
                fontSize: 10, padding: '2px 6px', background: 'transparent',
                border: '1px solid var(--border-soft)', borderRadius: 3,
                cursor: 'pointer', color: 'var(--muted)', fontFamily: 'var(--font-mono)',
              }}
              onClick={() => onMerge(a)}
              title="Merge with similar activities"
            >
              ⊕ merge
            </button>
          )}
          <FacActions size="sm" onEdit={() => onEdit(a)} onRemove={() => onRemove(a.id)} />
        </span>
      </div>
      ...
      {/* Expandable sources section for merged result cards */}
      {isMergedResult && (
        <div style={{ marginTop: 8 }}>
          ...
          >
            {showSources ? '▲' : '▼'} {showSources ? 'Hide' : 'Show'} {sources.length} source activit{sources.length === 1 ? 'y' : 'ies'}
          </button>
          {showSources && (
            <div style={{ marginTop: 8, display: 'grid', gap: 5 }}>
              {sources.map(src => (
                <div key={src.id} style={{
                  padding: '7px 10px', background: 'rgba(255,255,255,0.55)',
                  border: '1px solid var(--border-soft)', borderRadius: 4,
                  opacity: 0.72,
                }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 3 }}>
                    <span className="wa-avatar is-sm" style={{ background: src.participantColor, color: '#fff', opacity: 0.8 }}>{src.participantInitials}</span>
                    <span style={{ fontSize: 11, color: 'var(--muted)' }}>{src.participantName}</span>
                    <span className="wa-chip is-ghost" style={{ fontSize: 9, padding: '1px 4px', marginLeft: 'auto' }}>merged source</span>
                  </div>
                  <div style={{ fontSize: 12.5, color: 'var(--ink-2)', lineHeight: 1.3 }}>{src.title}</div>
                  <div style={{ marginTop: 4 }}>
                    <ActivityChipsShort tpo={src.tpo} freq={src.freq} energy={src.energy} />
                  </div>
                </div>
              ))}
            </div>
...
function LiveView({ activities, allActivities, participants, perPerson, onEdit, onRemove, onMerge }: {
  activities: Activity[];          // already filtered (no isMergedSource)
  allActivities: Activity[];       // full set including sources, used to find sources for merged results
  ...
  // Build lookup: mergedResultId -> source activities (for expandable source sections)
  const sourcesMap = new Map<string, Activity[]>();
  allActivities.filter(a => a.isMergedSource && a.mergedIntoId).forEach(src => {
    const list = sourcesMap.get(src.mergedIntoId!) ?? [];
    list.push(src);
    sourcesMap.set(src.mergedIntoId!, list);
  });
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '260px 1fr 1fr', height: '100%' }}>
      ...
      <main style={{ padding: '24px 28px', overflowY: 'auto', background: 'var(--cream)', borderRight: '1px solid var(--rule)' }}>
        ...
        {recent.length === 0 ? (
          ...
          <div style={{ display: 'grid', gap: 8 }}>
            {recent.map(a => (
              <LiveCard key={a.id} a={a} onEdit={onEdit} onRemove={onRemove} onMerge={onMerge} sources={sourcesMap.get(a.id)} />
            ))}
          </div>
        )}
      </main>
...
function MatrixView({ activities, selected, onSelect, onEdit, onRemove, onFlag, onClassify }: {
  activities: Activity[];
        ...
        {sel ? (
          <>
            ...
            <div style={{ marginTop: 10, display: 'flex', gap: 6, justifyContent: 'center' }}>
              <FacActions onEdit={() => onEdit(sel)} onRemove={() => onRemove(sel.id)} />
            </div>
            {sel.discussionNote && (
...
function GroupedView({ activities, onEdit, onRemove }: {
  activities: Activity[];
  onEdit: (a: Activity) => void;
  onRemove: (id: string) => void;
}) {
          ...
          return (
            <div key={key} style={{
              ...
              <div style={{ display: 'grid', gap: 6, overflowY: 'auto', paddingRight: 4 }}>
                ...
                {g.items.map((a, idx) => (
                  <GroupedCard key={a.id} a={a} rank={isManual ? undefined : idx + 1} onEdit={onEdit} onRemove={onRemove} />
                ))}
              </div>
            </div>
          );
...
function GroupedCard({ a, rank, onEdit, onRemove }: {
  a: Activity;
  rank?: number;
  onEdit: (a: Activity) => void;
  onRemove: (id: string) => void;
...
function DiscussView({
  ...
  onClassify, onFlag, onNote, onEdit, onRemove,
}: {
  activities: Activity[];
  classified: Activity[];
  pending: Activity[];
          ...
          <>
            <div style={{ padding: '18px 20px 14px', borderBottom: '1px solid var(--border-soft)' }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, alignItems: 'center' }}>
                ...
                <span style={{ marginLeft: 'auto' }}>
                  <FacActions size="sm" onEdit={() => onEdit(foc)} onRemove={() => onRemove(foc.id)} />
                </span>
              </div>
            </div>
...
function MergeModal({
  ...
  onToggle, onTitle, onTpo, onFreq, onEnergy,
  onConfirm, onClose,
}: {
  anchor: Activity;
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 50 }}>
      ...
      <div style={{
        ...
        <div style={{ padding: '16px 22px', borderBottom: '1px solid var(--border-soft)', display: 'flex', alignItems: 'center', gap: 14 }}>
          <div style={{ flex: 1 }}>
            <div className="wa-eyebrow" style={{ color: 'var(--amber)' }}>↳ Facilitator action · merge activities</div>
            <h2 className="wa-display" style={{ fontSize: 20, margin: '4px 0 0', fontWeight: 500 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', flex: 1, overflow: 'hidden' }}>
          ...
          <div style={{ padding: '18px 20px', borderRight: '1px solid var(--border-soft)', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div>
              <div className="wa-eyebrow" style={{ marginBottom: 8 }}>↳ Anchor activity (always included)</div>
              <div style={{
                padding: '10px 12px', background: 'var(--amber-bg)',
                border: '2px solid var(--amber)', borderRadius: 5,
              }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 5 }}>
                  <span className="wa-avatar is-sm" style={{ background: anchor.participantColor, color: '#fff' }}>{anchor.participantInitials}</span>
                  <span style={{ fontSize: 12, color: 'var(--muted)' }}>{anchor.participantName}</span>
                  <EffortPill tpo={anchor.tpo} freq={anchor.freq} size="sm" />
                </div>
                <div style={{ fontSize: 13.5, fontWeight: 500, lineHeight: 1.3 }}>{anchor.title}</div>
                <div style={{ marginTop: 5 }}>
                  <ActivityChipsShort tpo={anchor.tpo} freq={anchor.freq} energy={anchor.energy} />
                </div>
              </div>
            </div>
            ...
            <div>
              <div className="wa-eyebrow" style={{ marginBottom: 4 }}>
                ↳ Select activities to merge
              </div>
              {candidates.length > 0 && (
                <div style={{ fontSize: 11, color: 'var(--muted)', marginBottom: 10 }}>
                  Similarity score shown — title weight 85%, frequency 10%, time/occ 5%.
                </div>
              )}
              <div style={{ display: 'grid', gap: 6 }}>
                {candidates.map(({ activity: c, score }) => {
                  ...
                  return (
                    <button
                      ...
                      style={{
                        ...
                        border: `${on ? '2px' : '1px'} solid ${on ? 'var(--ink)' : 'var(--border-soft)'}`,
                    ...
                    >
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 5 }}>
                        ...
                        <span style={{ fontSize: 12, color: 'var(--muted)' }}>{c.participantName}</span>
                        <EffortPill tpo={c.tpo} freq={c.freq} size="sm" />
                        {score > 0.15 && (
                          <span
                            className={`wa-chip ${score > 0.5 ? 'is-rust' : score > 0.3 ? 'is-amber' : ''}`}
                            style={{ marginLeft: 'auto', fontSize: 10, padding: '1px 5px', fontWeight: 600 }}
                          >
                            {Math.round(score * 100)}% match
                          </span>
                        )}
                        <span style={{ fontSize: 13, marginLeft: 4 }}>{on ? '☑' : '☐'}</span>
                      </div>
                      <div style={{ fontSize: 13, fontWeight: 500, lineHeight: 1.3 }}>{c.title}</div>
                      <div style={{ marginTop: 4 }}>
                        <ActivityChipsShort tpo={c.tpo} freq={c.freq} energy={c.energy} />
                      </div>
                    </button>
                  );
                })}
            ...
            <div style={{ marginTop: 4, padding: '10px 12px', background: 'var(--paper-deep)', borderRadius: 4, fontSize: 11.5, color: 'var(--ink-2)', lineHeight: 1.5 }}>
              <span className="wa-mono" style={{ fontSize: 10, color: 'var(--muted)', display: 'block', marginBottom: 3 }}>↳ NOTE · PERMANENT</span>
              Merge is permanent — no undo. Source activities are hidden from facilitator views but remain in the data for engineer boards and exports.
            </div>
          </div>

          {/* Right — merged result fields */}
          <div style={{ padding: '18px 20px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div>
              <div className="wa-eyebrow" style={{ marginBottom: 8 }}>↳ Merged result · consolidated title</div>
              <div className="wa-mono" style={{ fontSize: 10.5, color: 'var(--muted-2)', marginBottom: 8 }}>
                The merged activity will be unclassified and needs re-classification after merging.
              </div>
              <label className="wa-label">Title</label>
              <input
                className="wa-input"
                style={{ fontSize: 14, padding: '10px 13px', marginBottom: 4 }}
                value={title}
                onChange={e => onTitle(e.target.value)}
                placeholder="Consolidated activity title…"
              />
            </div>
                ...
                {([['<30m','<30m'],['30m-2h','30m–2h'],['half-day','½ day'],['day+','1+ day']] as [TimePerOccurrence, string][]).map(([v, lbl]) => {
                  ...
                  return (
                    <button key={v} onClick={() => onTpo(v)} style={{
                      flex: 1, padding: '8px 6px', background: on ? 'var(--ink)' : '#fff',
                      color: on ? 'var(--cream)' : 'var(--ink)', border: '1px solid ' + (on ? 'var(--ink)' : 'var(--rule)'),
            ...
            <div>
              ...
              <div style={{ display: 'flex', gap: 5 }}>
                {([['daily','Daily'],['weekly','Wkly'],['monthly','Mthly'],['quarterly','Qtrly'],['adhoc','Ad hoc']] as [Frequency, string][]).map(([v, lbl]) => {
                  const on = v === freq;
                  return (
                    <button key={v} onClick={() => onFreq(v)} style={{
                      flex: 1, padding: '8px 4px', background: on ? 'var(--ink)' : '#fff',
                      color: on ? 'var(--cream)' : 'var(--ink)', border: '1px solid ' + (on ? 'var(--ink)' : 'var(--rule)'),
            ...
            <div style={{ padding: '10px 12px', background: '#fff', border: '1px solid var(--border-soft)', borderRadius: 4, marginTop: 4 }}>
              <div className="wa-eyebrow" style={{ marginBottom: 6 }}>Preview</div>
              <div style={{ fontSize: 14, fontWeight: 500, marginBottom: 6 }}>{title || <span style={{ color: 'var(--muted-2)', fontStyle: 'italic' }}>No title yet</span>}</div>
              <EffortPill tpo={tpo} freq={freq} />
              <div style={{ marginTop: 6 }}>
                <ActivityChipsShort tpo={tpo} freq={freq} energy={energy} />
              </div>
              <div style={{ marginTop: 8, fontSize: 11, color: 'var(--muted)' }}>
                Merging <strong>{selected.size}</strong> activit{selected.size === 1 ? 'y' : 'ies'} · result will be unclassified
              </div>
            </div>
          </div>
        </div>
        ...
        <div style={{ padding: '14px 22px', borderTop: '1px solid var(--border-soft)', display: 'flex', gap: 10, alignItems: 'center' }}>
          <div style={{ flex: 1, fontSize: 12, color: 'var(--muted)' }}>
            {!canConfirm && <span style={{ color: 'var(--amber)' }}>Select at least 2 activities to merge (anchor + 1 more).</span>}
            {canConfirm && <span style={{ color: 'var(--ink-2)' }}>Ready to merge {selected.size} activities into one.</span>}
          </div>
          <button className="wa-btn is-ghost" style={{ padding: '8px 14px', fontSize: 13 }} onClick={onClose}>Cancel</button>
          <button
            className="wa-btn is-rust"
            style={{ padding: '8px 18px', fontSize: 13, opacity: canConfirm ? 1 : 0.4, cursor: canConfirm ? 'pointer' : 'not-allowed' }}
            onClick={canConfirm ? onConfirm : undefined}
            disabled={!canConfirm}
          >
            ⊕ Merge {selected.size} activities
          </button>
        </div>
      </div>
    </div>
