...
nextApp.prepare().then(() => {
  ...
  app.post('/api/sessions/:id/merge', (req, res) => {
    ...
    if (!Array.isArray(sourceIds) || sourceIds.length < 2) return res.status(400).json({ error: 'at least 2 sourceIds required' });

    // Validate all source IDs exist
    const sources = sourceIds.map((sid: string) => session.activities.get(sid)).filter(Boolean) as typeof Array.prototype;
    if (sources.length !== sourceIds.length) return res.status(400).json({ error: 'one or more source activities not found' });

    const sourceList = sources as Array<import('./store').Activity>;
    const first = sourceList[0];

    // Create merged activity
    const mergedId = crypto.randomUUID().slice(0, 8);
    const merged: import('./store').Activity = {
      id: mergedId,
      sessionId: session.id,
    ...
    for (const sid of sourceIds) {
      const src = session.activities.get(sid);
      if (src) {
    ...
    io.to(req.params.id).emit('activity:merged', { merged, sourceIds });
    return res.json({ merged, sourceIds });
  });
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('activity:merge', (data: {
      ...
      tpo: TimePerOccurrence; freq: Frequency; energy: Energy; token: string;
    }) => {
      const session = sessions.get(data.sessionId);
      if (!session || data.token !== session.facilitatorToken) return;
      if (!Array.isArray(data.sourceIds) || data.sourceIds.length < 2) return;
      ...
      io.to(data.sessionId).emit('activity:merged', { merged, sourceIds: data.sourceIds });
    });
