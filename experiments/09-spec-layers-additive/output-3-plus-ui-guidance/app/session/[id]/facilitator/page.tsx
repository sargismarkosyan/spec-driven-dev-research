...
export default function FacilitatorView({ params }: { params: { id: string } }) {
  ...
  // Merge modal state
  const [mergeSourceActivity, setMergeSourceActivity] = useState<Activity | null>(null);
  const [mergeSelected, setMergeSelected]             = useState<Set<string>>(new Set());
  const [mergeTitleVal, setMergeTitleVal]             = useState('');
  const [mergeTpoVal, setMergeTpoVal]                 = useState<TimePerOccurrence>('30m-2h');
  const [mergeFreqVal, setMergeFreqVal]               = useState<Frequency>('weekly');
  const [mergeEnergyVal, setMergeEnergyVal]           = useState<Energy>('fine');
  ...
  useEffect(() => {
    ...
    socket.on('activity:merged', ({ mergedActivity, updatedSources }: {
      mergedActivity: Activity; updatedSources: Activity[];
    }) => {
      setActivities(prev => {
        const withUpdatedSources = prev.map(x => {
          const updated = updatedSources.find(s => s.id === x.id);
          return updated ?? x;
        });
        // Add the new merged activity if not already present
        if (withUpdatedSources.some(x => x.id === mergedActivity.id)) return withUpdatedSources;
        return [...withUpdatedSources, mergedActivity];
      });
    });
  ...
  const openMerge = (a: Activity) => {
    setMergeSourceActivity(a);
    setMergeSelected(new Set([a.id]));
    setMergeTitleVal(a.title);
    setMergeTpoVal(a.tpo);
    setMergeFreqVal(a.freq);
    setMergeEnergyVal(a.energy);
  };

  const handleMerge = () => {
    if (!mergeSourceActivity || mergeSelected.size < 2) return;
    socket.emit('activity:merge', {
      ...
      tpo: mergeTpoVal,
      freq: mergeFreqVal,
      energy: mergeEnergyVal,
      token,
    });
    setMergeSourceActivity(null);
  };
  ...
  // All activities (for engineer board, export — not filtered)
  const activeActivities = activities;
  // Facilitator views filter out merged source activities
  const facActivities = activities.filter(a => !a.isMergedSource);

  const flagged    = facActivities.filter(a => a.flagged);
  const classified = facActivities.filter(a => a.teamAuto !== 'unclassified');
  const pending    = facActivities.filter(a => a.teamAuto === 'unclassified');

  const discusActivity = pending[discussIdx] ?? pending[0] ?? null;
  const perPerson = new Map<string, number>();
  // Use all activities for per-person counts (to reflect actual submissions)
  activeActivities.filter(a => !a.isMergedSource).forEach(a => perPerson.set(a.participantName, (perPerson.get(a.participantName) ?? 0) + 1));
  ...
  return (
    <div className="wa-layout">
      ...
      <div className="wa-topbar">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          ...
          <div className="wa-eyebrow" style={{ fontSize: 10 }}>
            FACILITATOR · {facActivities.length} activities
            {classified.length > 0 ? ` · ${classified.length}/${facActivities.length} classified` : ''}
      ...
      <div style={{ flex: 1, overflow: 'hidden', position: 'relative' }}>
        {view === 'live'    && <LiveView activities={facActivities} allActivities={activities} participants={participants} perPerson={perPerson} onEdit={openEdit} onRemove={handleRemove} onMerge={openMerge} />}
        {view === 'matrix'  && <MatrixView activities={facActivities} selected={selectedActivity} onSelect={setSelectedActivity} onEdit={openEdit} onRemove={handleRemove} onFlag={handleFlag} onClassify={handleClassify} onMerge={openMerge} />}
        {view === 'grouped' && <GroupedView activities={facActivities} onEdit={openEdit} onRemove={handleRemove} onMerge={openMerge} />}
        {view === 'discuss' && (
          <DiscussView
            activities={facActivities}
      ...
      {mergeSourceActivity && (
        <MergeModal
          sourceActivity={mergeSourceActivity}
          allActivities={facActivities}
          selected={mergeSelected}
          onToggle={(actId) => {
            if (actId === mergeSourceActivity.id) return; // source can't be deselected
          ...
          freq={mergeFreqVal}
          energy={mergeEnergyVal}
          onTitle={setMergeTitleVal}
          onTpo={setMergeTpoVal}
          onFreq={setMergeFreqVal}
          onEnergy={setMergeEnergyVal}
          onMerge={handleMerge}
          onClose={() => setMergeSourceActivity(null)}
        />
      )}
...
function LiveCard({ a, allActivities, onEdit, onRemove, onMerge }: {
  a: Activity;
  ...
  const isMergedResult = !!(a.mergedSourceIds && a.mergedSourceIds.length > 0);
  const sourceCards = isMergedResult && allActivities
    ? allActivities.filter(x => a.mergedSourceIds!.includes(x.id))
    : [];

  return (
    <div className="wa-activity">
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
        {isMergedResult && sourceCards.length > 0 ? (
          <MergedAvatarStack activities={sourceCards} />
        ) : (
        ...
        {isMergedResult && <span className="wa-chip is-sage" style={{ fontSize: 10, padding: '1px 6px' }}>⇄ merged</span>}
        <span style={{ marginLeft: 'auto' }}>
          <MergeFacActions size="sm" onEdit={() => onEdit(a)} onRemove={() => onRemove(a.id)} onMerge={() => onMerge(a)} />
        </span>
      </div>
      ...
      {isMergedResult && sourceCards.length > 0 && (
        <div style={{ marginTop: 8 }}>
          <button
            ...
            style={{ background: 'transparent', border: 'none', cursor: 'pointer', fontSize: 10, color: 'var(--muted)', fontFamily: 'var(--font-mono)', padding: '2px 0' }}
          >
            {showSources ? '▲ hide sources' : `▼ ${sourceCards.length} source cards`}
          </button>
          {showSources && (
            <div style={{ display: 'grid', gap: 4, marginTop: 6 }}>
              {sourceCards.map(src => (
                <div key={src.id} style={{ padding: '6px 10px', background: 'var(--paper-deep)', borderRadius: 4, opacity: 0.65, border: '1px solid var(--border-soft)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3 }}>
                    <span className="wa-avatar is-sm" style={{ background: src.participantColor, color: '#fff', width: 16, height: 16, fontSize: 8 }}>{src.participantInitials}</span>
                    <span style={{ fontSize: 10, color: 'var(--muted)' }}>{src.participantName.split(' ')[0]}</span>
                  </div>
...
function LiveView({ activities, allActivities, participants, perPerson, onEdit, onRemove, onMerge }: {
  activities: Activity[];
  allActivities: Activity[];
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '260px 1fr 1fr', height: '100%' }}>
      ...
      <main style={{ padding: '24px 28px', overflowY: 'auto', background: 'var(--cream)', borderRight: '1px solid var(--rule)' }}>
        ...
        {recent.length === 0 ? (
          ...
          <div style={{ display: 'grid', gap: 8 }}>
            {recent.map(a => (
              <LiveCard key={a.id} a={a} allActivities={allActivities} onEdit={onEdit} onRemove={onRemove} onMerge={onMerge} />
            ))}
          </div>
        )}
      </main>
        ...
        {sel ? (
          <>
            ...
            <div style={{ marginTop: 10, display: 'flex', gap: 6, justifyContent: 'center' }}>
              <MergeFacActions onEdit={() => onEdit(sel)} onRemove={() => onRemove(sel.id)} onMerge={() => onMerge(sel)} />
            </div>
            {sel.discussionNote && (
          ...
          <>
            <div style={{ padding: '18px 20px 14px', borderBottom: '1px solid var(--border-soft)' }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, alignItems: 'center' }}>
                ...
                <span style={{ marginLeft: 'auto' }}>
                  <MergeFacActions size="sm" onEdit={() => onEdit(foc)} onRemove={() => onRemove(foc.id)} onMerge={() => onMerge(foc)} />
                </span>
              </div>
            </div>
...
// ══════════════════════════════════════════════════════════════════════════════
// MERGE FAC ACTIONS — edit / merge / remove buttons
// ══════════════════════════════════════════════════════════════════════════════

function MergeFacActions({
  size = 'md',
  onEdit,
  onMerge,
  onRemove,
...
function MergedAvatarStack({ activities }: { activities: Activity[] }) {
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', position: 'relative', height: 22 }}>
      {activities.map((a, i) => (
        <span
          key={a.id}
          className="wa-avatar is-sm"
          title={a.participantName}
          style={{
            background: a.participantColor,
            color: '#fff',
            border: '2px solid #fff',
            marginLeft: i === 0 ? 0 : -8,
            position: 'relative',
            zIndex: activities.length - i,
          }}
        >
...
// ══════════════════════════════════════════════════════════════════════════════
// SIMILARITY COMPUTATION (client-side, Jaccard)
// ══════════════════════════════════════════════════════════════════════════════
...
function computeSimilarity(source: Activity, candidate: Activity): number {
  const ta = tokenize(source.title);
  const tb = tokenize(candidate.title);
  const titleSim = jaccardIndex(ta, tb);
  const freqMatch = source.freq === candidate.freq ? 1 : 0;
  const tpoMatch = source.tpo === candidate.tpo ? 1 : 0;
  return 0.85 * titleSim + 0.10 * freqMatch + 0.05 * tpoMatch;
}
...
function MergeModal({
  ...
  // Compute candidates: all non-isMergedSource, non-source activities, with similarity
  const candidates = allActivities
    .filter(a => !a.isMergedSource && a.id !== sourceActivity.id)
    .map(a => ({ activity: a, score: computeSimilarity(sourceActivity, a) }))
    .sort((a, b) => b.score - a.score);

  // Filter by search
  const q = search.trim().toLowerCase();
  const filtered = candidates.filter(({ activity: a }) => {
    if (!q) return true;
    return (
  ...
  // Selected activities (for merged card preview)
  const selectedActivities = allActivities.filter(a => selected.has(a.id));

  // Total effort h/wk for selected
  const totalEffort = selectedActivities.reduce((sum, a) => {
    const { hrs } = calcEffort(a.tpo, a.freq);
    return sum + hrs;
  }, 0);
  ...
  // First names for author line
  const firstNames = selectedActivities.map(a => a.participantName.split(' ')[0]);

  // Similarity badge style
  const badgeStyle = (score: number): React.CSSProperties => {
    if (score > 0.6) return { background: 'var(--rust-bg)', color: 'var(--rust)', border: '1px solid #e8c8b8' };
    if (score > 0.3) return { background: 'var(--amber-bg)', color: '#7a5010', border: '1px solid #e8d2a8' };
    return { background: 'var(--paper-deep)', color: 'var(--muted)', border: '1px solid var(--border-soft)' };
  };

  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 60 }}>
      <div
        style={{ position: 'absolute', inset: 0, background: 'rgba(28,26,22,0.45)', backdropFilter: 'blur(2px)' }}
        onClick={onClose}
      />
      <div style={{
        ...
        <div style={{ padding: '16px 22px', borderBottom: '1px solid var(--border-soft)', display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <button
            onClick={onClose}
            style={{ background: 'transparent', border: 0, fontSize: 20, color: 'var(--muted)', cursor: 'pointer', padding: 4 }}
          >×</button>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', flex: 1, overflow: 'hidden' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', borderRight: '1px solid var(--border-soft)', overflow: 'hidden' }}>
            <div style={{ padding: '14px 18px 10px', borderBottom: '1px solid var(--border-soft)', flexShrink: 0 }}>
              <span className="wa-eyebrow">Select cards to merge · {n} selected</span>
            </div>

            {/* Source card (always selected) */}
            <div style={{ padding: '10px 18px', borderBottom: '1px solid var(--border-soft)', background: 'var(--paper)', flexShrink: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <span className="wa-avatar is-sm" style={{ background: sourceActivity.participantColor, color: '#fff' }}>
                  {sourceActivity.participantInitials}
                </span>
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div className="wa-mono" style={{ fontSize: 10, color: 'var(--muted)' }}>
                    {sourceActivity.participantName.split(' ')[0]} · {TPO_SHORT[sourceActivity.tpo]} · {FREQ_SHORT[sourceActivity.freq]}
                  </div>
                </div>
                <span className="wa-chip is-ghost" style={{ fontSize: 10, padding: '1px 6px', whiteSpace: 'nowrap', flexShrink: 0 }}>starting card</span>
              </div>
            </div>

            {/* Search */}
            <div style={{ padding: '10px 18px', borderBottom: '1px solid var(--border-soft)', flexShrink: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: '#fff', border: '1px solid var(--rule)', borderRadius: 4, padding: '6px 10px' }}>
                ...
                <input
                  ...
                  style={{
                    ...
                    fontFamily: 'var(--font-body)', background: 'transparent', color: 'var(--ink)',
                  }}
                />
                {search && (
                  <button
                    onClick={() => setSearch('')}
                    style={{ background: 'transparent', border: 'none', cursor: 'pointer', color: 'var(--muted)', fontSize: 14, padding: 0, lineHeight: 1 }}
                  >×</button>
                )}
              </div>
            </div>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '8px 18px' }}>
              {filtered.length === 0 && q ? (
                <div style={{ padding: '16px 0', fontSize: 12, color: 'var(--muted-2)', fontStyle: 'italic' }}>
                  No activities match &apos;{search}&apos;.
                </div>
              ) : filtered.length === 0 ? (
                <div style={{ padding: '16px 0', fontSize: 12, color: 'var(--muted-2)', fontStyle: 'italic' }}>
                  No other activities in this session.
                </div>
              ) : (
                <div style={{ display: 'grid', gap: 4 }}>
                  {filtered.map(({ activity: a, score }) => {
                    ...
                    return (
                      <div
                        ...
                        style={{
                          ...
                          background: isChecked ? 'var(--paper)' : '#fff',
                          border: `1px solid ${isChecked ? 'var(--rule)' : 'var(--border-soft)'}`,
                        }}
                      >
                        <span style={{
                          width: 16, height: 16, borderRadius: 3, flexShrink: 0,
                          background: isChecked ? 'var(--ink)' : '#fff',
                          border: `1px solid ${isChecked ? 'var(--ink)' : 'var(--muted-2)'}`,
                          display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
                          color: '#fff', fontSize: 11,
                        }}>{isChecked ? '✓' : ''}</span>
                        <span className="wa-avatar is-sm" style={{ background: a.participantColor, color: '#fff', flexShrink: 0 }}>
                          {a.participantInitials}
                        </span>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <div className="wa-mono" style={{ fontSize: 10, color: 'var(--muted)' }}>
                            {a.participantName.split(' ')[0]} · {TPO_SHORT[a.tpo]} · {FREQ_SHORT[a.freq]}
                          </div>
                        </div>
                        {score > 0.15 && (
                          <span style={{
          ...
          <div style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            <div style={{ padding: '14px 18px 10px', borderBottom: '1px solid var(--border-soft)', flexShrink: 0 }}>
              <span className="wa-eyebrow">Merged card — edit before creating</span>
            </div>

            <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px' }}>
              {/* Stacked avatars + author line */}
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 18 }}>
                <MergedAvatarStack activities={selectedActivities} />
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 12, color: 'var(--muted)' }}>
                    {firstNames.join(' + ')}
                  </div>
                </div>
                <span className="wa-chip is-sage" style={{ fontSize: 10, padding: '2px 8px', whiteSpace: 'nowrap', flexShrink: 0 }}>
                  ⇄ {n} cards · ~{totalEffort.toFixed(1)} h/wk
                </span>
              </div>

              {/* Title */}
              <div style={{ marginBottom: 16 }}>
                <label className="wa-label" style={{ display: 'block', marginBottom: 4 }}>Title</label>
                <input
                  className="wa-input"
                  value={title}
              ...
              <div style={{ marginBottom: 14 }}>
                <label className="wa-label" style={{ display: 'block', marginBottom: 6 }}>Time per occurrence</label>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 5 }}>
                  {([['<30m','<30 min'],['30m-2h','30m–2h'],['half-day','Half day'],['day+','1+ day']] as [TimePerOccurrence, string][]).map(([v, lbl]) => {
                    const on = v === tpo;
                    return (
                      <button key={v} onClick={() => onTpo(v)} style={{
                        ...
                        border: '1px solid ' + (on ? 'var(--ink)' : 'var(--rule)'), borderRadius: 4,
              ...
              {/* Frequency */}
              <div style={{ marginBottom: 14 }}>
                <label className="wa-label" style={{ display: 'block', marginBottom: 6 }}>Frequency</label>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr 1fr', gap: 5 }}>
                  {([['daily','Daily'],['weekly','Weekly'],['monthly','Monthly'],['quarterly','Qtrly'],['adhoc','Ad hoc']] as [Frequency, string][]).map(([v, lbl]) => {
                    const on = v === freq;
                    return (
              ...
              <div style={{ marginBottom: 18 }}>
                <label className="wa-label" style={{ display: 'block', marginBottom: 6 }}>Energy</label>
              ...
              <div style={{ padding: '10px 12px', background: 'var(--paper-deep)', border: '1px dashed var(--border-soft)', borderRadius: 4, fontSize: 11.5, color: 'var(--muted)', lineHeight: 1.5 }}>
        ...
        {/* Footer */}
        <div style={{ padding: '12px 22px', borderTop: '1px solid var(--border-soft)', display: 'flex', alignItems: 'center', gap: 14, background: 'var(--paper)' }}>
          <span style={{ flex: 1, fontSize: 12.5, color: canMerge ? 'var(--ink-2)' : 'var(--muted)', fontStyle: canMerge ? 'normal' : 'italic' }}>
            {canMerge
              ? `Creating 1 merged card from ${n} originals.`
          ...
          <button
            ...
            style={{ padding: '8px 18px', fontSize: 13, opacity: canMerge ? 1 : 0.45, cursor: canMerge ? 'pointer' : 'not-allowed' }}
            disabled={!canMerge}
            onClick={onMerge}
          >
            ⇄ Create merged card →
