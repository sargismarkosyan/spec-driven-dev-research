...
nextApp.prepare().then(() => {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('activity:merge', (data: {
      ...
      const sources = data.sourceIds.map(sid => session.activities.get(sid)).filter(Boolean) as import('./store').Activity[];
      if (sources.length < 2) return;

      // Create merged result using first source's participant identity
      ...
      const mergedActivity: import('./store').Activity = {
        ...
        createdAt: new Date(),
        editHistory: [],
        mergedSourceIds: data.sourceIds,
      };
      ...
      io.to(data.sessionId).emit('activity:merged', {
        mergedActivity,
        updatedSources,
      });
    });
