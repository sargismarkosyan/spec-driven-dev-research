# Merge Feature — Spec Extract

All merge-related spec content compiled from across the full spec, organized by ablation category. Each section is labelled so it's clear what gets removed in each run.
...
The merged result's `teamAuto` is always set to `unclassified`, regardless of what the source activities were classified as. The team must re-classify the merged activity during discussion.
...
A "⇄ merge" facilitator action button appears on activity cards in: live view, matrix view, grouped view, and discuss view. Clicking it opens the merge modal with that activity pre-selected as the source card.
...
3. Closes modal by setting `mergeSource` to null
...
3. Update source activity records in local state to reflect `isMergedSource: true` and `mergedIntoId` (for traceability rendering)
...
|---|---|---|
| > 0.60 | `is-rust` | High confidence match |
| > 0.30 (up to 0.60) | `is-amber` | Moderate confidence match |
| ≤ 0.30 | No accent (neutral chip) | Low confidence match |
...
- Overlay: semi-transparent dark with 2px backdrop blur

### Merged Card Avatar Stack

Avatars stacked horizontally, each offset 8px left of the previous, with a 2px white border between them.
...
|---|---|
| `isMergedSource` | `true` |
| `mergedIntoId` | ID of the newly created merged activity |
...
**Exception:** In the facilitator live view, the merged result card can be expanded to reveal its source cards in a dimmed state, providing traceability.
