...
import sessions, {
  ...
  titleSimilarity, mergeSimilarity,
...
nextApp.prepare().then(() => {
  ...
  // Merge candidates (programmatic access — browser computes locally)
  app.get('/api/sessions/:id/activities/:aid/merge-candidates', (req, res) => {
    ...
    const candidates = Array.from(session.activities.values())
      ...
      .map(a => {
        const sem = titleSimilarity(source.title, a.title);
        const sameFreq = source.freq === a.freq;
        const sameTpo  = source.tpo  === a.tpo;
        const similarity = mergeSimilarity(source, a);
  ...
  app.post('/api/sessions/:id/activities/merge', (req, res) => {
    ...
    const mergedActivity: Activity = {
      ...
      tpo: tpo ?? first.tpo,
      freq: freq ?? first.freq,
      energy: energy ?? first.energy,
      teamAuto: 'unclassified',
      flagged: false,
      discussionNote: '',
      createdAt: new Date(),
      editHistory: [],
      mergedFromIds:      sources.map(a => a.id),
    ...
    session.activities.set(mergedId, mergedActivity);
