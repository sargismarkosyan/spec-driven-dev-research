...
export default function FacilitatorView({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    ...
    socket.on('activity:merged', ({ newActivity, updatedSources }: { newActivity: Activity; updatedSources: Activity[] }) => {
      setActivities(prev => {
        ...
        if (withUpdatedSources.some(x => x.id === newActivity.id)) return withUpdatedSources;
        return [...withUpdatedSources, newActivity];
      });
    });
  ...
  // Per-person counts based on non-merged original activities only
  activities.filter(a => !a.isMergedSource && !a.mergedFromIds).forEach(a =>
    perPerson.set(a.participantName, (perPerson.get(a.participantName) ?? 0) + 1)
  );
  ...
  return (
    <div className="wa-layout">
      ...
      <div style={{ flex: 1, overflow: 'hidden', position: 'relative' }}>
        {view === 'live'    && <LiveView activities={activeActivities} participants={participants} perPerson={perPerson} onEdit={openEdit} onRemove={handleRemove} onMerge={openMerge} />}
      ...
      {mergeSource && (
        <MergeModal
          ...
          onToggle={(id) => setMergeSelected(prev =>
            prev.includes(id) ? (id === mergeSource.id ? prev : prev.filter(x => x !== id)) : [...prev, id]
          )}
          title={mergeTitle}
          tpo={mergeTpo}
          freq={mergeFreq}
...
function LiveCard({ a, onEdit, onRemove, onMerge }: {
  a: Activity;
    ...
    <div className="wa-activity">
      ...
      {a.mergedFromIds && a.mergedFromIds.length > 0 && (
        <div style={{ marginTop: 8, paddingTop: 8, borderTop: '1px dashed var(--border-soft)' }}>
          <span className="wa-mono" style={{ fontSize: 9, color: 'var(--muted-2)', letterSpacing: '0.06em' }}>
            ⇄ MERGED FROM {a.mergedFromIds.length} CARDS ·{' '}
          </span>
          {a.mergedFromNames?.map((name, i) => (
            <span key={i} className="wa-avatar is-sm" style={{ background: a.mergedFromColors?.[i] ?? '#888', color: '#fff', marginRight: 3, fontSize: 8, opacity: 0.7 }}>
              {a.mergedFromInitials?.[i] ?? '?'}
            </span>
          ))}
        </div>
      )}
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '260px 1fr 1fr', height: '100%' }}>
      ...
      <main style={{ padding: '24px 28px', overflowY: 'auto', background: 'var(--cream)', borderRight: '1px solid var(--rule)' }}>
        ...
        {recent.length === 0 ? (
          ...
          <div style={{ display: 'grid', gap: 8 }}>
            {recent.map(a => (
              <LiveCard key={a.id} a={a} onEdit={onEdit} onRemove={onRemove} onMerge={onMerge} />
            ))}
          </div>
        )}
      </main>
...
function DiscussView({
  ...
  // Client-side similarity scoring (Jaccard word overlap + attribute bonus)
  const computeSimilarity = (a: Activity, b: Activity): number => {
    const tokenise = (s: string) =>
      new Set(s.toLowerCase().split(/\W+/).filter(w => w.length > 2));
    const aW = tokenise(a.title);
    const bW = tokenise(b.title);
    const inter = new Set([...aW].filter(w => bW.has(w)));
    const union = new Set([...aW, ...bW]);
    const titleScore = union.size > 0 ? inter.size / union.size : 0;
    let attr = 0;
    if (a.tpo    === b.tpo)    attr += 0.10;
    if (a.freq   === b.freq)   attr += 0.10;
    if (a.energy === b.energy) attr += 0.05;
    return Math.min(1, titleScore * 0.75 + attr);
  };
          ...
          <>
            ...
            {foc && (() => {
              const similars = activities
                ...
                .map(a => ({ activity: a, score: computeSimilarity(foc, a) }))
              ...
              return (
                <div style={{ padding: '14px 16px', borderBottom: '1px solid var(--border-soft)' }}>
                  <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 8 }}>
                    ...
                    <span className="wa-mono" style={{ fontSize: 10, color: 'var(--muted-2)' }}>merge?</span>
                  </div>
                  <div style={{ display: 'grid', gap: 5 }}>
                    {similars.map(({ activity: a, score }) => {
                      const pct = Math.round(score * 100);
                      const badgeTone = pct > 60 ? 'is-rust' : pct > 30 ? 'is-amber' : '';
                      return (
                        <div key={a.id} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 8px', background: '#fff', border: '1px solid var(--border-soft)', borderRadius: 4 }}>
                          <span className="wa-avatar is-sm" style={{ background: a.participantColor, color: '#fff' }}>{a.participantInitials}</span>
                          <span style={{ flex: 1, fontSize: 11, lineHeight: 1.3, minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{a.title}</span>
                          <span className={`wa-chip ${badgeTone}`} style={{ fontSize: 9, padding: '1px 5px', flexShrink: 0 }}>{pct}%</span>
                          <button
                            style={{ fontSize: 9, padding: '2px 6px', background: 'transparent', border: '1px solid var(--border-soft)', borderRadius: 3, cursor: 'pointer', color: 'var(--muted)', fontFamily: 'var(--font-mono)', flexShrink: 0 }}
                            onClick={() => onMerge(foc)}
                          >
                            ⇄ merge
                          </button>
...
function MergeModal({
  source, allActivities, selected, onToggle,
  title, tpo, freq, energy, search,
  onTitle, onTpo, onFreq, onEnergy, onSearch,
  onMerge, onClose,
}: {
  source: Activity;
  ...
  const searchRef = (el: HTMLInputElement | null) => { if (el) el.focus(); };
  ...
  // Candidates: non-source, non-merged-source activities, sorted by similarity desc
  const searchLower = search.toLowerCase();
  const candidates: MergeCandidate[] = allActivities
    .filter(a => a.id !== source.id && !a.isMergedSource)
    .map(a => ({ activity: a, score: computeSimilarity(source, a) }))
    .filter(({ activity: a }) =>
      !searchLower ||
      a.title.toLowerCase().includes(searchLower) ||
      a.participantName.toLowerCase().includes(searchLower)
    )
    .sort((a, b) => b.score - a.score);

  // Merged card preview effort
  const selectedActivities = allActivities.filter(a => selected.includes(a.id));
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 60 }}>
      <div style={{ position: 'absolute', inset: 0, background: 'rgba(28,26,22,0.55)', backdropFilter: 'blur(2px)' }} onClick={onClose} />
      <div style={{
        position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)',
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', flex: 1, overflow: 'hidden' }}>
          ...
          <div style={{ borderRight: '1px solid var(--border-soft)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            <div style={{ padding: '14px 18px 10px', borderBottom: '1px solid var(--border-soft)' }}>
              <div className="wa-eyebrow" style={{ marginBottom: 10 }}>Select cards to merge · {selected.length} selected</div>
              {/* Source card — always selected */}
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '10px 12px', background: 'var(--ink)', borderRadius: 4, marginBottom: 10 }}>
                <span style={{ width: 16, height: 16, borderRadius: 3, background: 'var(--rust)', border: '2px solid var(--rust)', flexShrink: 0, marginTop: 2, display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }}>
                  <span style={{ color: '#fff', fontSize: 10, fontWeight: 700 }}>✓</span>
                </span>
                <span className="wa-avatar is-sm" style={{ background: source.participantColor, color: '#fff', flexShrink: 0 }}>{source.participantInitials}</span>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--cream)', lineHeight: 1.3 }}>{source.title}</div>
                  <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.6)', marginTop: 2 }}>{source.participantName} · {calcEffort(source.tpo, source.freq).display}</div>
                </div>
                <span className="wa-chip is-ghost" style={{ fontSize: 9, padding: '1px 6px', color: 'rgba(255,255,255,0.5)', border: '1px solid rgba(255,255,255,0.2)', background: 'transparent', flexShrink: 0 }}>starting card</span>
              </div>
              {/* Search */}
              <input
                ref={searchRef}
            ...
            <div style={{ overflowY: 'auto', flex: 1, padding: '10px 14px' }}>
              {candidates.length === 0 ? (
                <div style={{ fontSize: 12, color: 'var(--muted-2)', fontStyle: 'italic', paddingTop: 8 }}>No other activities found.</div>
              ) : (
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        padding: '9px 10px', borderRadius: 4, cursor: 'pointer', marginBottom: 4,
                        background: isChecked ? 'var(--rust-bg)' : '#fff',
                        border: `1px solid ${isChecked ? '#e8c8b8' : 'var(--border-soft)'}`,
                      }}
                    >
                      <span style={{
                        width: 14, height: 14, borderRadius: 3, flexShrink: 0, marginTop: 2,
                        border: `1.5px solid ${isChecked ? 'var(--rust)' : 'var(--muted-2)'}`,
                        background: isChecked ? 'var(--rust)' : '#fff',
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 2 }}>{a.participantName} · {calcEffort(a.tpo, a.freq).display}</div>
                      </div>
                      {pct > 0 && (
                        <span className={`wa-chip ${badgeTone}`} style={{ fontSize: 9, padding: '1px 5px', flexShrink: 0 }}>{pct}%</span>
                      )}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            <div style={{ padding: '14px 18px 10px', borderBottom: '1px solid var(--border-soft)' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
                ...
                <span className="wa-chip is-amber" style={{ fontSize: 10, marginLeft: 'auto' }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '14px 18px' }}>
              ...
              <div style={{ marginBottom: 12 }}>
                ...
                <input
                  className="wa-input"
                  style={{ fontSize: 14, fontWeight: 700, padding: '10px 12px' }}
                  value={title}
              ...
              <div style={{ marginBottom: 10 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 4 }}>
                  {([['<30m','<30m'],['30m-2h','30m–2h'],['half-day','½ day'],['day+','1+ day']] as const).map(([v, lbl]) => {
                    const on = v === tpo;
                    return (
              ...
              <div style={{ marginBottom: 10 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr 1fr', gap: 4 }}>
                  {([['daily','Daily'],['weekly','Wkly'],['monthly','Mthly'],['quarterly','Qtrly'],['adhoc','Adhoc']] as const).map(([v, lbl]) => {
                    const on = v === freq;
                    return (
                      <button key={v} onClick={() => onFreq(v)} style={{
                        padding: '7px 4px', fontSize: 10, textAlign: 'center',
                        background: on ? 'var(--ink)' : '#fff', color: on ? 'var(--cream)' : 'var(--ink)',
                        border: '1px solid ' + (on ? 'var(--ink)' : 'var(--rule)'),
                        borderRadius: 4, cursor: 'pointer',
                      }}>{lbl}</button>
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 4 }}>
                  {([['energizing','Energizes'],['fine','Fine'],['tedious','Tedious'],['draining','Drains']] as const).map(([v, lbl]) => {
                    const on = v === energy;
                    return (
              ...
              <div style={{ padding: '10px 12px', background: 'var(--paper)', borderRadius: 4, border: '1px dashed var(--rule)' }}>
        ...
        <div style={{ padding: '12px 22px', borderTop: '1px solid var(--border-soft)', display: 'flex', alignItems: 'center', gap: 12 }}>
          <span style={{ fontSize: 12, color: canMerge ? 'var(--ink-2)' : 'var(--muted)' }}>
            {canMerge
