...
import sessions, {
  ...
  AVATAR_COLORS, makeInitials, effortHrsPerWk, activitySimilarity,
...
nextApp.prepare().then(() => {
  ...
  // Merge candidates (programmatic access)
  ...
  app.post('/api/sessions/:id/activities/merge', (req, res) => {
    ...
    if (!sourceIds || sourceIds.length < 2) return res.status(400).json({ error: 'at least 2 sourceIds required' });

    const sources: Activity[] = [];
    for (const sid of sourceIds) {
      const a = session.activities.get(sid);
      if (!a) return res.status(404).json({ error: `activity not found: ${sid}` });
      sources.push(a);
    }
    ...
    const merged: Activity = {
      id: mergedId,
      sessionId: session.id,
      participantId: session.facilitatorId,
      participantName: session.facilitatorName,
      participantInitials: makeInitials(session.facilitatorName),
      participantColor: AVATAR_COLORS[0],
      title: title ?? firstSource.title,
      tpo: tpo ?? firstSource.tpo,
      freq: freq ?? firstSource.freq,
      energy: energy ?? firstSource.energy,
      teamAuto: 'unclassified',
      flagged: false,
      discussionNote: '',
      createdAt: new Date(),
    ...
    const updatedSources: Activity[] = sources.map(s => {
      s.isMergedSource = true;
      s.mergedIntoId = mergedId;
      return s;
    });

    const payload = { newActivity: merged, updatedSources };
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('activity:merge', (data: {
      ...
      title?: string; tpo: TimePerOccurrence; freq: Frequency; energy: Energy; token: string;
    }) => {
      const session = sessions.get(data.sessionId);
      if (!session || data.token !== session.facilitatorToken) return;
      ...
      const merged: Activity = {
        ...
        participantInitials: makeInitials(session.facilitatorName),
        participantColor: AVATAR_COLORS[0],
        title: data.title ?? firstSource.title,
        tpo: data.tpo,
        freq: data.freq,
        energy: data.energy,
        teamAuto: 'unclassified',
      ...
      io.to(data.sessionId).emit('activity:merged', { newActivity: merged, updatedSources });
    });
