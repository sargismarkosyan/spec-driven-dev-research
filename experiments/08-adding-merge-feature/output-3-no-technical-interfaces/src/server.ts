...
// NOTE: relatedTo is used by activity:relate socket handler; it lives on Activity as optional field.
...
nextApp.prepare().then(() => {
  ...
  // Merge activities (facilitator only)
  app.post('/api/sessions/:id/merge', (req, res) => {
    ...
    if (!sourceIds || sourceIds.length < 2) return res.status(400).json({ error: 'at least 2 source IDs required' });

    // Validate all source IDs exist
    const sources = sourceIds.map(sid => session.activities.get(sid)).filter(Boolean) as Activity[];
    if (sources.length !== sourceIds.length) return res.status(400).json({ error: 'one or more source IDs not found' });

    const first = sources[0];

    // Build aggregated reporter lists from all sources
    ...
    // reportedBy = all unique reporters across all sources (use names as keys)
    const seen = new Set<string>();
    const reportedBy: string[]         = [];
    const reportedByInitials: string[] = [];
    const reportedByColors: string[]   = [];
    for (const src of sources) {
      if (!seen.has(src.participantName)) {
        seen.add(src.participantName);
        reportedBy.push(src.participantName);
        reportedByInitials.push(src.participantInitials);
        reportedByColors.push(src.participantColor);
      }
    }
    ...
    session.activities.set(mergedId, merged);

    // Mark each source as consumed
    for (const src of sources) {
      src.isMergedSource = true;
      src.mergedIntoId   = mergedId;
      io.to(req.params.id).emit('activity:updated', src);
    }

    io.to(req.params.id).emit('activity:added', merged);
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('activity:merge', (data: {
      ...
      const session = sessions.get(data.sessionId);
      if (!session || data.token !== session.facilitatorToken) return;
      if (!data.sourceIds || data.sourceIds.length < 2) return;

      const sources = data.sourceIds.map(sid => session.activities.get(sid)).filter(Boolean) as Activity[];
      if (sources.length !== data.sourceIds.length) return;
      ...
      for (const src of sources) {
        src.isMergedSource = true;
        src.mergedIntoId   = mergedId;
        io.to(data.sessionId).emit('activity:updated', src);
      }

      io.to(data.sessionId).emit('activity:added', merged);
    });
