...
export type Activity = {
  ...
  // Merge tracking — source activity (consumed by a merge)
...
/**
 * Tokenise a title for Jaccard similarity:
 *  1. Lowercase
 *  2. Remove non-alphanumeric characters
 *  3. Split into words
 *  4. Discard words with length ≤ 2
 */
...
export function combinedSimilarity(
  ...
  aFreq: Frequency, bFreq: Frequency,
  aTpo: TimePerOccurrence, bTpo: TimePerOccurrence,
): number {
  const titleSim = jaccardSimilarity(tokenise(aTitle), tokenise(bTitle));
  const sameFreq = aFreq === bFreq ? 1 : 0;
  const sameTpo  = aTpo  === bTpo  ? 1 : 0;
