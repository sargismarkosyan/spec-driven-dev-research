...
// ── Client-side similarity helpers (Jaccard + combined score) ────────────────

function tokeniseTitle(text: string): Set<string> {
  return new Set(
    text
      .toLowerCase()
      .replace(/[^a-z0-9\s]/g, ' ')
...
function activitySimilarity(a: Activity, b: Activity): number {
  const titleSim = jaccardSimilarity(tokeniseTitle(a.title), tokeniseTitle(b.title));
...
type Activity = {
  ...
  // Merge tracking — source (consumed)
...
export default function FacilitatorView({ params }: { params: { id: string } }) {
  ...
  const handleMerge = useCallback(() => {
    if (!mergeSource || mergeChecked.length < 2) return;
    socket.emit('activity:merge', {
      sessionId: id,
      sourceIds: mergeChecked,
      title: mergeTitle,
      tpo: mergeTpo,
      freq: mergeFreq,
      energy: mergeEnergy,
    ...
    setMergeSource(null);
  }, [id, token, mergeSource, mergeChecked, mergeTitle, mergeTpo, mergeFreq, mergeEnergy]);
  ...
  // Merged sources are hidden from facilitator views (visible on engineer board + export)
  ...
  return (
    <div className="wa-layout">
      ...
      <div style={{ flex: 1, overflow: 'hidden', position: 'relative' }}>
        {view === 'live'    && <LiveView activities={activeActivities} allActivities={activities} participants={participants} perPerson={perPerson} onEdit={openEdit} onMerge={openMerge} onRemove={handleRemove} />}
      ...
      {mergeSource && (
        <MergeModal
          ...
          freq={mergeFreq}
          energy={mergeEnergy}
          onCheckedChange={setMergeChecked}
...
function LiveCard({ a, onEdit, onMerge, onRemove }: {
  a: Activity;
  onEdit: (a: Activity) => void;
  onMerge: (a: Activity) => void;
  onRemove: (id: string) => void;
}) {
  // Show dimmed source cards inside merged result cards
  return (
    <div className="wa-activity">
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
        ...
        {a.mergedFromIds && a.mergedFromIds.length > 0 && (
          <span className="wa-chip is-ghost" style={{ fontSize: 10, padding: '2px 6px' }}>⇄ {a.mergedFromIds.length} merged</span>
        )}
...
function LiveView({ activities, allActivities, participants, perPerson, onEdit, onMerge, onRemove }: {
  activities: Activity[];       // non-merged-source (facilitator view)
  allActivities: Activity[];    // all activities including merged sources
      ...
      <main style={{ padding: '24px 28px', overflowY: 'auto', background: 'var(--cream)', borderRight: '1px solid var(--rule)' }}>
        ...
        {recent.length === 0 ? (
          ...
          <div style={{ display: 'grid', gap: 8 }}>
            {recent.map(a => (
              <div key={a.id}>
                <LiveCard a={a} onEdit={onEdit} onMerge={onMerge} onRemove={onRemove} />
                {/* Expandable section showing source cards for merged results */}
                {a.mergedFromIds && a.mergedFromIds.length > 0 && (() => {
                  const srcs = a.mergedFromIds.map(sid => allActivities.find(x => x.id === sid)).filter(Boolean) as Activity[];
                  if (srcs.length === 0) return null;
                  return (
                    <details style={{ marginTop: 2 }}>
                      <summary style={{ fontSize: 11, color: 'var(--muted)', cursor: 'pointer', padding: '3px 8px', listStyle: 'none', display: 'flex', alignItems: 'center', gap: 6 }}>
                        <span className="wa-mono" style={{ fontSize: 10 }}>↳ {srcs.length} source cards</span>
                        <span style={{ fontSize: 10, color: 'var(--muted-2)' }}>▾</span>
                      </summary>
                      <div style={{ display: 'grid', gap: 4, marginTop: 4, paddingLeft: 12 }}>
                        {srcs.map(src => (
                          <div key={src.id} className="wa-activity" style={{ opacity: 0.5, padding: '8px 10px', fontSize: 12 }}>
                            ...
                            <div style={{ fontSize: 12.5, fontWeight: 500, marginTop: 4 }}>{src.title}</div>
                          </div>
                        ))}
                      </div>
                    </details>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '14px 16px' }}>
              ...
              {foc && (() => {
                const similars = activities
                  ...
                  .map(a => ({ a, score: activitySimilarity(foc, a) }))
                ...
                return (
                  <div style={{ marginBottom: 14 }}>
                    <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 8 }}>
                      ...
                      <span className="wa-mono" style={{ fontSize: 10, color: 'var(--muted-2)' }}>merge candidates</span>
                    </div>
                    <div style={{ display: 'grid', gap: 5 }}>
                      {similars.map(({ a, score }) => {
                        const badgeTone = score > 0.60 ? 'is-rust' : score > 0.30 ? 'is-amber' : '';
                        return (
                          <div key={a.id} style={{ background: '#fff', border: '1px solid var(--border-soft)', borderRadius: 4, padding: '8px 10px' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                              ...
                              <span style={{ fontSize: 11, color: 'var(--muted)', flex: 1 }}>{a.participantName.split(' ')[0]}</span>
                              <span className={`wa-chip ${badgeTone}`} style={{ fontSize: 9, padding: '1px 5px' }}>{Math.round(score * 100)}%</span>
                            </div>
                            <div style={{ fontSize: 12, fontWeight: 500, lineHeight: 1.3, marginBottom: 5 }}>{a.title}</div>
                            <button
                              style={{ fontSize: 10, padding: '3px 7px', background: 'transparent', border: '1px solid var(--border-soft)', borderRadius: 3, cursor: 'pointer', color: 'var(--muted)', fontFamily: 'var(--font-mono)' }}
                              onClick={() => onMerge(foc)}
                            >
                              ⇄ merge
                            </button>
                    ...
                    <hr className="wa-rule" style={{ margin: '12px 0 8px' }} />
                  </div>
                );
              })()}
              <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 8 }}>
...
function MergeModal({
  source, allActivities,
  checked, title, tpo, freq, energy,
  onCheckedChange, onTitle, onTpo, onFreq, onEnergy,
  onMerge, onClose,
}: {
  source: Activity;
  allActivities: Activity[];
  ...
  // Candidates: all non-merged-source activities except source itself, sorted by similarity desc
  const candidates = allActivities
    .filter(a => a.id !== source.id && !a.isMergedSource)
    .map(a => ({ a, score: activitySimilarity(source, a) }))
    .sort((x, y) => y.score - x.score);
  ...
  const filtered = search.trim() === ''
    ...
    : candidates.filter(({ a }) => {
        ...
        return a.title.toLowerCase().includes(q) || a.participantName.toLowerCase().includes(q);
      });

  const toggleCandidate = (id: string) => {
    if (id === source.id) return; // source always checked
    if (checked.includes(id)) {
      onCheckedChange(checked.filter(x => x !== id));
    } else {
      onCheckedChange([...checked, id]);
    }
  };

  // Merged card preview: stacked avatars from checked sources
  const checkedActivities = checked.map(cid => allActivities.find(a => a.id === cid)).filter(Boolean) as Activity[];
  const totalHrsWk = checkedActivities.reduce((sum, a) => {
    const TPO_H: Record<string, number> = { '<30m': 0.5, '30m-2h': 1.25, 'half-day': 4, 'day+': 8 };
    const FREQ_W: Record<string, number> = { daily: 5, weekly: 1, monthly: 0.23, quarterly: 0.077, adhoc: 0.3 };
    return sum + (TPO_H[a.tpo] ?? 1) * (FREQ_W[a.freq] ?? 1);
  }, 0);
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 60 }}>
      <div
        style={{ position: 'absolute', inset: 0, background: 'rgba(28,26,22,0.5)', backdropFilter: 'blur(2px)' }}
        onClick={onClose}
      />
      <div style={{
        position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)',
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', flex: 1, overflow: 'hidden' }}>
          ...
          <div style={{ borderRight: '1px solid var(--rule)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            {/* Source card */}
            <div style={{ padding: '14px 18px', borderBottom: '1px solid var(--border-soft)', background: 'var(--paper)' }}>
              <div className="wa-eyebrow" style={{ marginBottom: 10 }}>
                Select cards to merge · {checked.length} selected
              </div>
              {/* Source card (always checked) */}
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '10px 12px', background: '#fff', border: '1px solid var(--rule)', borderRadius: 4 }}>
                <span style={{
                  width: 18, height: 18, borderRadius: 3, background: 'var(--ink)', border: '1px solid var(--ink)',
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                    ...
                    <span style={{ fontSize: 12, fontWeight: 500 }}>{source.title}</span>
                  </div>
                ...
                <span className="wa-chip is-ghost" style={{ fontSize: 9, padding: '2px 5px', flexShrink: 0 }}>starting card</span>
              </div>
            </div>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '8px 18px 14px' }}>
              {filtered.length === 0 && (
                <div style={{ fontSize: 12, color: 'var(--muted-2)', fontStyle: 'italic', paddingTop: 12 }}>No matching activities.</div>
              )}
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      background: isChecked ? 'var(--paper-deep)' : '#fff',
                  ...
                  >
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 3 }}>
                        <span className="wa-avatar is-sm" style={{ background: a.participantColor, color: '#fff' }}>{a.participantInitials}</span>
                        <span style={{ fontSize: 12.5, fontWeight: 500, flex: 1, lineHeight: 1.3 }}>{a.title}</span>
                        {score > 0.15 && (
                          <span className={`wa-chip ${badgeTone}`} style={{ fontSize: 9, padding: '1px 5px', flexShrink: 0 }}>{Math.round(score * 100)}%</span>
                        )}
                      </div>
                      <div className="wa-mono" style={{ fontSize: 10, color: 'var(--muted-2)' }}>
          ...
          {/* Right column — editable merged card preview */}
          <div style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            <div style={{ padding: '14px 18px', borderBottom: '1px solid var(--border-soft)', background: 'var(--paper)' }}>
              <div className="wa-eyebrow" style={{ marginBottom: 10 }}>Merged card — edit before creating</div>
              {/* Avatar stack */}
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                <div style={{ display: 'flex' }}>
                  {checkedActivities.slice(0, 5).map((a, i) => (
                    <span
                      key={a.id}
                      className="wa-avatar is-sm"
                      style={{ background: a.participantColor, color: '#fff', marginLeft: i === 0 ? 0 : -8, border: '2px solid var(--cream)', zIndex: checkedActivities.length - i }}
                    >
                ...
                <div style={{ flex: 1, fontSize: 12, color: 'var(--muted)' }}>
                  {checkedActivities.map(a => a.participantName.split(' ')[0]).join(', ')}
                </div>
              </div>
              <span className="wa-chip" style={{ fontSize: 10, padding: '2px 7px' }}>
                ⇄ {checked.length} cards · ~{totalHrsWk.toFixed(1)} h/wk
              </span>
            </div>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '14px 18px' }}>
                  ...
                  {([['<30m','<30m'], ['30m-2h','30m–2h'], ['half-day','½ day'], ['day+','1+ day']] as [TimePerOccurrence, string][]).map(([v, lbl]) => {
                    ...
                    return (
                      <button key={v} onClick={() => onTpo(v)} style={{
                        padding: '8px 4px', background: on ? 'var(--ink)' : '#fff', color: on ? 'var(--cream)' : 'var(--ink)',
                        border: '1px solid ' + (on ? 'var(--ink)' : 'var(--rule)'), borderRadius: 4, cursor: 'pointer', fontSize: 12, fontWeight: on ? 500 : 400,
                      }}>{lbl}</button>
                    );
                  })}
                </div>
                  ...
                  {([['daily','Daily'], ['weekly','Weekly'], ['monthly','Mthly'], ['quarterly','Qtrly'], ['adhoc','Ad hoc']] as [Frequency, string][]).map(([v, lbl]) => {
                    ...
                    return (
                      <button key={v} onClick={() => onFreq(v)} style={{
                        padding: '8px 4px', background: on ? 'var(--ink)' : '#fff', color: on ? 'var(--cream)' : 'var(--ink)',
                        border: '1px solid ' + (on ? 'var(--ink)' : 'var(--rule)'), borderRadius: 4, cursor: 'pointer', fontSize: 11, fontWeight: on ? 500 : 400,
                      }}>{lbl}</button>
                    );
                  })}
                </div>
              ...
              <div style={{ padding: '10px 12px', background: 'var(--paper)', border: '1px solid var(--border-soft)', borderRadius: 4, fontSize: 11.5, color: 'var(--muted)', lineHeight: 1.5 }}>
        ...
        {/* Footer */}
        <div style={{ padding: '12px 22px', borderTop: '1px solid var(--border-soft)', display: 'flex', alignItems: 'center', gap: 12 }}>
          <span style={{ fontSize: 12.5, color: canMerge ? 'var(--ink-2)' : 'var(--muted)', flex: 1 }}>
            {canMerge
              ? `Creating 1 merged card from ${checked.length} originals.`
