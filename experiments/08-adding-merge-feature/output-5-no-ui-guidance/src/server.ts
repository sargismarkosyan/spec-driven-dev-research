...
nextApp.prepare().then(() => {
  ...
  // Merge candidates (programmatic access only — not called by browser)
  app.get('/api/sessions/:id/activities/:aid/merge-candidates', (req, res) => {
    ...
    const candidates = Array.from(session.activities.values())
      ...
      .map(a => ({ ...a, score: combinedSimilarity(source, a) }))
  ...
  app.post('/api/sessions/:id/activities/merge', (req, res) => {
    ...
    if (!tpo || !freq || !energy) return res.status(400).json({ error: 'tpo, freq, energy required' });

    const sources = sourceIds.map((id: string) => session.activities.get(id)).filter(Boolean) as Activity[];
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('activity:merge', (data: {
      ...
      const sources = data.sourceIds.map(id => session.activities.get(id)).filter(Boolean) as Activity[];
