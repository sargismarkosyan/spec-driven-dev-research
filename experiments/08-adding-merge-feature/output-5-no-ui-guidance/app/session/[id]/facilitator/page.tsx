...
// ── Client-side similarity helpers ───────────────────────────────────────────
...
function computeCombinedScore(
  a: { title: string; tpo: string; freq: string },
  b: { title: string; tpo: string; freq: string },
): number {
...
type MergeCandidate = Activity & { score: number };
...
export default function FacilitatorView({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    ...
    socket.on('activity:merged', ({ newActivity, updatedSources }: { newActivity: Activity; updatedSources: Activity[] }) => {
      setActivities(prev => {
        // Update sources in state (mark as isMergedSource)
        const updatedMap = new Map(updatedSources.map(s => [s.id, s]));
        const filtered = prev.map(a => updatedMap.has(a.id) ? updatedMap.get(a.id)! : a);
        // Remove any old entry with same newActivity id, then add the new merged result
        return [...filtered.filter(a => a.id !== newActivity.id), newActivity];
      });
    });
  ...
  const openMerge = (source: Activity) => {
    // Compute candidates client-side — no network call
    const candidates: MergeCandidate[] = activities
      .filter(a => a.id !== source.id && !a.isMergedSource)
      .map(a => ({ ...a, score: computeCombinedScore(source, a) }))
      .sort((a, b) => b.score - a.score);
    setMergeSource(source);
    setMergeCandidates(candidates);
    setMergeSelected([source.id]);
    setMergeTitle(source.title);
    setMergeTpo(source.tpo);
    setMergeFreq(source.freq);
    setMergeEnergy(source.energy);
    setMergeSearch('');
  };

  const handleMerge = () => {
    if (!mergeSource || mergeSelected.length < 2) return;
  ...
  // isMergedSource activities are excluded from all facilitator list views/counts/trays
  ...
  return (
    <div className="wa-layout">
      ...
      <div style={{ flex: 1, overflow: 'hidden', position: 'relative' }}>
        {view === 'live'    && <LiveView activities={activeActivities} allActivities={activities} participants={participants} perPerson={perPerson} onEdit={openEdit} onRemove={handleRemove} onMerge={openMerge} />}
      ...
      {mergeSource && (
        <MergeModal
          ...
          onToggleCandidate={(id) => {
            if (id === mergeSource.id) return; // source cannot be deselected
          ...
          onMergeTpo={setMergeTpo}
          onMergeFreq={setMergeFreq}
          onMergeEnergy={setMergeEnergy}
...
function LiveCard({ a, allActivities, onEdit, onRemove, onMerge }: {
  ...
  const sourcedCards = isMerged
    ? (a.mergedFromIds ?? []).map(id => allActivities.find(x => x.id === id)).filter(Boolean) as Activity[]
    : [];
  ...
  return (
    <div className="wa-activity">
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
        ...
        {isMerged && (
          <span className="wa-chip" style={{ fontSize: 9, padding: '1px 5px', background: 'var(--paper-deep)', color: 'var(--muted)', fontFamily: 'var(--font-mono)' }}>
            merged · {sourcedCards.length} sources
          </span>
        )}
      ...
      {isMerged && sourcedCards.length > 0 && (
        <div style={{ marginTop: 8 }}>
          <button
            onClick={() => setExpanded(v => !v)}
            style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 0, fontSize: 10, color: 'var(--muted)', fontFamily: 'var(--font-mono)', letterSpacing: '0.04em' }}
          >
            {expanded ? '▲ hide sources' : `▼ show ${sourcedCards.length} merged sources`}
          </button>
          {expanded && (
            <div style={{ marginTop: 6, display: 'grid', gap: 4 }}>
              {sourcedCards.map(src => (
                <div key={src.id} style={{ padding: '7px 9px', background: 'var(--paper)', border: '1px solid var(--border-soft)', borderRadius: 4, opacity: 0.55 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 3 }}>
                    <span className="wa-avatar is-sm" style={{ background: src.participantColor, color: '#fff', width: 14, height: 14, fontSize: 8 }}>{src.participantInitials}</span>
                    <span style={{ fontSize: 10.5, color: 'var(--muted)' }}>{src.participantName}</span>
                  </div>
                  <div style={{ fontSize: 12, color: 'var(--ink-2)', lineHeight: 1.3 }}>{src.title}</div>
                </div>
              ))}
            </div>
          )}
          ...
          <>
            ...
            {(() => {
              const similar = activities
                .filter(a => a.id !== foc.id && !a.isMergedSource)
                .map(a => ({ ...a, score: computeCombinedScore(foc, a) }))
              ...
              return (
                <div style={{ padding: '14px 16px', borderBottom: '1px solid var(--border-soft)' }}>
                  ...
                  <div style={{ display: 'grid', gap: 5 }}>
                    {similar.map(s => (
                      <div key={s.id} style={{ padding: '6px 9px', background: '#fff', border: '1px solid var(--border-soft)', borderRadius: 4 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 2 }}>
                          <span className="wa-avatar is-sm" style={{ background: s.participantColor, color: '#fff', width: 14, height: 14, fontSize: 8 }}>{s.participantInitials}</span>
                          <span style={{ fontSize: 10.5, color: 'var(--muted)' }}>{s.participantName.split(' ')[0]}</span>
                          <span className="wa-chip" style={{ marginLeft: 'auto', fontSize: 9, padding: '1px 4px', background: 'var(--paper-deep)', color: 'var(--ink-2)', fontFamily: 'var(--font-mono)' }}>
                            {Math.round(s.score * 100)}% match
                          </span>
                        </div>
                        <div style={{ fontSize: 11.5, color: 'var(--ink-2)', lineHeight: 1.3 }}>{s.title}</div>
                      </div>
                    ))}
                  </div>
                </div>
...
function MergeModal({
  source, candidates, selected, onToggleCandidate,
  search, onSearch,
  mergeTitle, mergeTpo, mergeFreq, mergeEnergy,
  onMergeTitle, onMergeTpo, onMergeFreq, onMergeEnergy,
  onConfirm, onClose,
}: {
  source: Activity;
  ...
  mergeTitle: string;
  mergeTpo: TimePerOccurrence;
  mergeFreq: Frequency;
  mergeEnergy: Energy;
  onMergeTitle: (v: string) => void;
  onMergeTpo: (v: TimePerOccurrence) => void;
  onMergeFreq: (v: Frequency) => void;
  onMergeEnergy: (v: Energy) => void;
  onConfirm: () => void;
  onClose: () => void;
}) {
  const canConfirm = selected.length >= 2;

  // Total h/wk = sum of calcEffort(tpo, freq).hrs for all selected activities
  const allActivitiesMap = new Map<string, Activity>();
  allActivitiesMap.set(source.id, source);
  candidates.forEach(c => allActivitiesMap.set(c.id, c));
  const totalHrsPerWk = selected.reduce((sum, id) => {
    const a = allActivitiesMap.get(id);
    return a ? sum + calcEffort(a.tpo, a.freq).hrs : sum;
  }, 0);

  const filtered = candidates.filter(c => {
    if (!search) return true;
    const q = search.toLowerCase();
    return c.title.toLowerCase().includes(q) || c.participantName.toLowerCase().includes(q);
  });

  const statusText = canConfirm
    ? `Merging ${selected.length} activities into one`
    : `Select at least 1 more to merge (${selected.length - 1} / need 1+)`;
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 60 }}>
      ...
      <div style={{
        position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)',
        width: 820, maxHeight: '92dvh', background: 'var(--cream)',
        borderRadius: 8, boxShadow: '0 24px 80px rgba(0,0,0,0.32)',
        border: '1px solid var(--rule)', overflow: 'hidden', display: 'flex', flexDirection: 'column',
      }}>
        {/* Header */}
        <div style={{ padding: '16px 22px', borderBottom: '1px solid var(--border-soft)', display: 'flex', alignItems: 'center', gap: 14 }}>
          <div style={{ flex: 1 }}>
            <Eyebrow color="var(--slate)">↳ Facilitator action · merge activities</Eyebrow>
            <h2 className="wa-display" style={{ fontSize: 20, margin: '4px 0 0', fontWeight: 500 }}>
              Combine duplicate activities into one
            </h2>
          </div>
          <button onClick={onClose} style={{ background: 'transparent', border: 0, fontSize: 20, color: 'var(--muted)', cursor: 'pointer', padding: 4 }}>×</button>
        </div>
        ...
        <div style={{
          padding: '9px 22px',
          background: canConfirm ? 'var(--sage-bg)' : 'var(--paper-deep)',
          borderBottom: '1px solid var(--border-soft)',
          fontSize: 12, color: canConfirm ? '#2b4a1c' : 'var(--muted)',
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', flex: 1, overflow: 'hidden' }}>
          {/* Left — candidate list */}
          <div style={{ display: 'flex', flexDirection: 'column', borderRight: '1px solid var(--rule)', overflow: 'hidden' }}>
            <div style={{ padding: '12px 18px', borderBottom: '1px solid var(--border-soft)', flexShrink: 0 }}>
              <input
                className="wa-input"
            ...
            <div style={{ overflowY: 'auto', flex: 1, padding: '10px 14px' }}>
              {/* Source card — always selected, cannot deselect */}
              <div style={{ marginBottom: 8 }}>
                <div className="wa-mono" style={{ fontSize: 10, color: 'var(--muted-2)', letterSpacing: '0.06em', marginBottom: 5 }}>↳ SOURCE · ALWAYS SELECTED</div>
                <div style={{
                  padding: '10px 12px', background: 'var(--ink)', color: 'var(--cream)',
                  borderRadius: 5, border: '1px solid var(--ink)',
                }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                    <span className="wa-avatar is-sm" style={{ background: source.participantColor, color: '#fff', border: '1.5px solid rgba(255,255,255,0.3)' }}>{source.participantInitials}</span>
                    <span style={{ fontSize: 11.5, opacity: 0.75 }}>{source.participantName}</span>
                    <EffortPill tpo={source.tpo} freq={source.freq} size="sm" />
                  </div>
                  <div style={{ fontSize: 13, fontWeight: 500 }}>{source.title}</div>
                </div>
              </div>

              {/* Candidates */}
              <div className="wa-mono" style={{ fontSize: 10, color: 'var(--muted-2)', letterSpacing: '0.06em', marginBottom: 5, marginTop: 14 }}>
                ↳ CANDIDATES · click to select
              </div>
              {filtered.length === 0 && (
                <div style={{ fontSize: 12, color: 'var(--muted-2)', fontStyle: 'italic', padding: '8px 0' }}>
                  {search ? 'No matches.' : 'No other activities in this session.'}
                </div>
              )}
              <div style={{ display: 'grid', gap: 6 }}>
                {filtered.map(c => {
                  ...
                  return (
                    <button
                      key={c.id}
                      onClick={() => onToggleCandidate(c.id)}
                      style={{
                        padding: '10px 12px', textAlign: 'left', cursor: 'pointer',
                        background: isSelected ? 'var(--paper-deep)' : '#fff',
                        border: `1.5px solid ${isSelected ? 'var(--ink)' : 'var(--border-soft)'}`,
                    ...
                    >
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                        ...
                        <span style={{ fontSize: 11.5, color: 'var(--muted)' }}>{c.participantName}</span>
                        <EffortPill tpo={c.tpo} freq={c.freq} size="sm" />
                        {c.score > 0.15 && (
                          <span className="wa-chip" style={{ marginLeft: 'auto', fontSize: 9, padding: '1px 5px', background: 'var(--sage-bg)', color: '#2b4a1c', fontFamily: 'var(--font-mono)', fontWeight: 600 }}>
                            {Math.round(c.score * 100)}% match
                          </span>
                        )}
                      </div>
                      <div style={{ fontSize: 13, fontWeight: isSelected ? 500 : 400, color: 'var(--ink)' }}>{c.title}</div>
                    </button>
                  );
                })}
              </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            <div style={{ padding: '14px 18px', borderBottom: '1px solid var(--border-soft)', flexShrink: 0 }}>
              <Eyebrow>↳ Merged result preview</Eyebrow>
              <div className="wa-mono" style={{ fontSize: 10, color: 'var(--muted-2)', marginTop: 2 }}>
                Edit before confirming
              </div>
            </div>
            <div style={{ overflowY: 'auto', flex: 1, padding: '14px 18px' }}>
              <label className="wa-label">Activity title</label>
              <input
                className="wa-input"
                style={{ fontSize: 13, padding: '9px 11px', marginBottom: 14 }}
                value={mergeTitle}
              ...
              <div style={{ display: 'flex', gap: 4, marginBottom: 12 }}>
                {([['<30m','< 30 min'],['30m-2h','30m–2h'],['half-day','½ day'],['day+','1+ day']] as [TimePerOccurrence, string][]).map(([v, lbl]) => {
                  const on = v === mergeTpo;
                  return (
                    <button key={v} onClick={() => onMergeTpo(v)} style={{
                      flex: 1, padding: '7px 4px', fontSize: 11,
                      background: on ? 'var(--ink)' : '#fff', color: on ? 'var(--cream)' : 'var(--ink-2)',
              ...
              <div style={{ display: 'flex', gap: 4, marginBottom: 12 }}>
                {([['daily','Daily'],['weekly','Weekly'],['monthly','Mthly'],['quarterly','Qtrly'],['adhoc','Adhoc']] as [Frequency, string][]).map(([v, lbl]) => {
                  const on = v === mergeFreq;
                  return (
                    <button key={v} onClick={() => onMergeFreq(v)} style={{
              ...
              <div style={{ display: 'flex', gap: 4, marginBottom: 14 }}>
                {([['energizing','Energizes'],['fine','Fine'],['tedious','Tedious'],['draining','Drains']] as [Energy, string][]).map(([v, lbl]) => {
                  const on = v === mergeEnergy;
                  return (
                    <button key={v} onClick={() => onMergeEnergy(v)} style={{
                      flex: 1, padding: '7px 4px', fontSize: 11,
                      background: on ? 'var(--ink)' : '#fff', color: on ? 'var(--cream)' : 'var(--ink-2)',
                      border: `1px solid ${on ? 'var(--ink)' : 'var(--rule)'}`, borderRadius: 4, cursor: 'pointer',
                    }}>{lbl}</button>
              ...
              <hr className="wa-rule" style={{ margin: '8px 0 12px' }} />

              {/* Total effort preview */}
              <div style={{ padding: '10px 12px', background: 'var(--paper)', border: '1px solid var(--border-soft)', borderRadius: 5 }}>
                <div className="wa-mono" style={{ fontSize: 10, color: 'var(--muted-2)', marginBottom: 4 }}>↳ COMBINED EFFORT · ALL SELECTED</div>
                <div style={{ fontSize: 16, fontWeight: 600, fontFamily: 'var(--font-mono)', color: effortTone(totalHrsPerWk) === 'is-rust' ? 'var(--rust)' : effortTone(totalHrsPerWk) === 'is-amber' ? 'var(--amber)' : 'var(--sage)' }}>
                  {calcEffort(mergeTpo, mergeFreq).display}
                </div>
                <div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 2 }}>
                  Sum across {selected.length} selected: ~{totalHrsPerWk.toFixed(1)} h/wk
                </div>
              </div>

              <div style={{ marginTop: 12, padding: '9px 11px', background: 'var(--paper-deep)', border: '1px dashed var(--rule)', borderRadius: 5, fontSize: 11, color: 'var(--ink-2)', lineHeight: 1.5 }}>
                <span className="wa-mono" style={{ fontSize: 9, color: 'var(--muted-2)' }}>↳ NOTE</span><br />
                Merge is permanent. Source cards will be hidden from facilitator views but remain visible on engineer boards and in export.
              </div>
            </div>
          </div>
        </div>
        ...
        {/* Footer */}
        <div style={{ padding: '14px 22px', borderTop: '1px solid var(--border-soft)', display: 'flex', gap: 10, alignItems: 'center' }}>
          <span style={{ flex: 1, fontSize: 12, color: 'var(--muted)', fontFamily: 'var(--font-mono)' }}>
            {selected.length} selected · {candidates.filter(c => !selected.includes(c.id) && c.score > 0.15).length} high-similarity candidates remaining
          </span>
          <button className="wa-btn is-ghost" style={{ padding: '8px 14px', fontSize: 13 }} onClick={onClose}>Cancel</button>
          <button
            className="wa-btn is-rust"
            style={{ padding: '8px 18px', fontSize: 13, opacity: canConfirm ? 1 : 0.5, cursor: canConfirm ? 'pointer' : 'not-allowed' }}
            onClick={canConfirm ? onConfirm : undefined}
            disabled={!canConfirm}
          >
            ⇌ Merge {canConfirm ? `${selected.length} activities` : '(select more)'}
          </button>
        </div>
      </div>
    </div>
