...
nextApp.prepare().then(() => {
  ...
  // Merge candidates (programmatic access — sorted by similarity, up to 5)
  app.get('/api/sessions/:id/activities/:aid/merge-candidates', (req, res) => {
    ...
    const candidates = Array.from(session.activities.values())
      .filter(a => a.id !== source.id && !a.mergedIntoId)
      .map(a => ({ activity: a, score: activitySimilarity(source, a) }))
      .sort((a, b) => b.score - a.score)
      .slice(0, 5)
      .map(({ activity, score }) => ({ ...activity, similarityScore: score }));
  ...
  app.post('/api/sessions/:id/activities/merge', (req, res) => {
    ...
    if (!Array.isArray(sourceIds) || sourceIds.length < 2)
      return res.status(400).json({ error: 'at least 2 sourceIds required' });
    if (!tpo || !freq || !energy)
      return res.status(400).json({ error: 'tpo, freq, energy required' });

    const sources = sourceIds.map((sid: string) => session.activities.get(sid)).filter(Boolean) as Activity[];
    if (sources.length !== sourceIds.length)
      return res.status(404).json({ error: 'one or more source activities not found' });
    ...
    session.activities.set(mergedActivity.id, mergedActivity);

    const updatedSources: Activity[] = sources.map(a => {
      a.mergedIntoId = mergedActivity.id;
      return a;
    });

    const payload = { newActivity: mergedActivity, updatedSources };
    io.to(req.params.id).emit('activity:merged', payload);
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('activity:merge', (data: {
      sessionId: string; sourceIds: string[];
      title?: string; tpo: string; freq: string; energy: string; token: string;
    }) => {
      const session = sessions.get(data.sessionId);
      if (!session || data.token !== session.facilitatorToken) return;
      if (!Array.isArray(data.sourceIds) || data.sourceIds.length < 2) {
        socket.emit('error', 'at least 2 sourceIds required');
        return;
      }
      ...
      if (sources.length !== data.sourceIds.length) {
        socket.emit('error', 'one or more source activities not found');
        return;
      }
      const first = sources[0];
      const mergedActivity: Activity = {
        ...
        tpo: data.tpo as TimePerOccurrence,
        freq: data.freq as Frequency,
      ...
      io.to(data.sessionId).emit('activity:merged', { newActivity: mergedActivity, updatedSources });
    });
