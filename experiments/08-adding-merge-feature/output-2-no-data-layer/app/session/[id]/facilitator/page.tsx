...
export default function FacilitatorView({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    ...
    socket.on('activity:merged', ({ newActivity, updatedSources }: {
      ...
      setActivities(prev => {
        // Replace updated sources in place, then add the new merged activity
        const sourceIds = new Set(updatedSources.map(a => a.id));
        const filtered = prev.map(x => {
          const updated = updatedSources.find(s => s.id === x.id);
          return updated ?? x;
        });
        // Add newActivity if not already present
        if (!filtered.find(x => x.id === newActivity.id)) {
  ...
  const openMerge = (a: Activity) => {
    ...
    setMergeSelectedIds([a.id]);
  };

  const handleMerge = () => {
    if (!mergeSourceActivity || mergeSelectedIds.length < 2) return;
    socket.emit('activity:merge', {
      sessionId: id,
      sourceIds: mergeSelectedIds,
      title: mergeTitle,
      tpo: mergeTpo,
      freq: mergeFreq,
      energy: mergeEnergy,
  ...
  // Filter merged source activities out of facilitator views (spec: merged sources hidden from fac lists)
  const activeActivities = activities.filter(a => !a.mergedIntoId);
    ...
    <div className="wa-layout">
      ...
      {mergeSourceActivity && (
        <MergeModal
          ...
          onToggleId={(aid) => {
            if (aid === mergeSourceActivity.id) return; // source always selected
...
function LiveCard({ a, allActivities, onEdit, onRemove, onMerge }: {
  ...
  const isMergedResult = !!(a.mergeSourceIds && a.mergeSourceIds.length > 0);
  const sourcesCards = isMergedResult && allActivities
    ? (a.mergeSourceIds ?? []).map(sid => allActivities.find(x => x.id === sid)).filter(Boolean) as Activity[]
    : [];
  ...
  return (
    <div className="wa-activity">
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
        ...
        {isMergedResult && (
          <span className="wa-chip" style={{ fontSize: 10, padding: '2px 6px', color: 'var(--rust)', borderColor: '#e8c8b8', background: 'var(--rust-bg)' }}>
            ⇄ merged · {sourcesCards.length} sources
          </span>
        )}
      ...
      {isMergedResult && sourcesCards.length > 0 && (
        <div style={{ marginTop: 8 }}>
          ...
          >
            {showSources ? '▲ hide originals' : `▼ show ${sourcesCards.length} original${sourcesCards.length > 1 ? 's' : ''}`}
          </button>
          {showSources && (
            <div style={{ marginTop: 6, display: 'grid', gap: 4, paddingLeft: 8, borderLeft: '2px solid var(--border-soft)' }}>
              {sourcesCards.map(src => (
                <div key={src.id} style={{
                  padding: '6px 9px', background: 'var(--paper)', border: '1px solid var(--border-soft)',
                  ...
                  <div style={{ fontSize: 12, color: 'var(--ink-2)', lineHeight: 1.35 }}>{src.title}</div>
                </div>
              ))}
            </div>
          )}
...
function DiscussView({
  activities, allActivities, classified, pending, flagged,
  ...
  // Client-side similarity (mirrors server formula)
  ...
  function combinedScore(a: Activity, b: Activity): number {
    return 0.85 * jaccardTitle(a.title, b.title)
          ...
          <>
            ...
            {foc && (() => {
              const candidates = allActivities
                .filter(a => a.id !== foc.id && !a.mergedIntoId)
                .map(a => ({ a, score: combinedScore(foc, a) }))
              ...
              return (
                <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--border-soft)' }}>
                  <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 8 }}>
                    ...
                    <span className="wa-mono" style={{ fontSize: 11, color: 'var(--muted)' }}>{candidates.length}</span>
                  </div>
                  <div style={{ display: 'grid', gap: 5 }}>
                    {candidates.map(({ a, score }) => {
                      const badgeClass = score > 0.6 ? 'is-rust' : score > 0.3 ? 'is-amber' : '';
                      return (
                        <div key={a.id} style={{ background: '#fff', border: '1px solid var(--border-soft)', borderRadius: 4, padding: '7px 9px', display: 'flex', alignItems: 'center', gap: 8 }}>
                          <span className="wa-avatar is-sm" style={{ background: a.participantColor, color: '#fff', flexShrink: 0 }}>{a.participantInitials}</span>
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ fontSize: 11.5, fontWeight: 500, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{a.title}</div>
                            <div className="wa-mono" style={{ fontSize: 10, color: 'var(--muted)' }}>{a.participantName.split(' ')[0]}</div>
                          </div>
                          <span className={`wa-chip ${badgeClass}`} style={{ fontSize: 9.5, padding: '2px 5px', flexShrink: 0 }}>{Math.round(score * 100)}%</span>
                          <button
                            style={{ fontSize: 10, padding: '3px 6px', background: 'transparent', border: '1px solid var(--border-soft)', borderRadius: 3, cursor: 'pointer', color: 'var(--muted)', fontFamily: 'var(--font-mono)', flexShrink: 0 }}
                            onClick={() => onMerge(foc)}
                            title="Merge these activities"
                          >⇄ merge</button>
                        </div>
                      );
                    })}
...
// Client-side similarity helpers (duplicated here so MergeModal is self-contained)
function mergeTokenize(s: string): Set<string> {
  return new Set(s.toLowerCase().replace(/[^a-z0-9 ]/g, '').split(/\s+/).filter(w => w.length > 2));
}
function mergeJaccard(a: string, b: string): number {
  const sa = mergeTokenize(a);
  const sb = mergeTokenize(b);
  if (sa.size === 0 && sb.size === 0) return 0;
  const inter = [...sa].filter(x => sb.has(x)).length;
  return inter / new Set([...sa, ...sb]).size;
}
function mergeCombinedScore(a: Activity, b: Activity): number {
  return 0.85 * mergeJaccard(a.title, b.title)
    + 0.10 * (a.freq === b.freq ? 1 : 0)
    + 0.05 * (a.tpo  === b.tpo  ? 1 : 0);
}

function MergeModal({
  sourceActivity, allActivities,
  title, tpo, freq, energy, selectedIds,
  onTitle, onTpo, onFreq, onEnergy,
  onToggleId, onMerge, onClose,
}: {
  ...
  // Candidates = all non-merged-source activities except the source card, sorted by similarity desc
  const candidates = allActivities
    .filter(a => a.id !== sourceActivity.id && !a.mergedIntoId)
    .map(a => ({ a, score: mergeCombinedScore(sourceActivity, a) }))
    .sort((x, y) => y.score - x.score);

  const filtered = search.trim()
    ? candidates.filter(({ a }) =>
        a.title.toLowerCase().includes(search.toLowerCase()) ||
        a.participantName.toLowerCase().includes(search.toLowerCase())
      )
    : candidates;

  const nSelected = selectedIds.length;
  const canMerge = nSelected >= 2;

  // TPO / Freq / Energy grid options
  const TPO_OPTS: [TimePerOccurrence, string][] = [['<30m','<30m'],['30m-2h','30m-2h'],['half-day','½ day'],['day+','1+ day']];
  const FREQ_OPTS: [Frequency, string][] = [['daily','Daily'],['weekly','Weekly'],['monthly','Monthly'],['quarterly','Qtrly'],['adhoc','Ad hoc']];
  const ENERGY_OPTS: [Energy, string][] = [['energizing','Energizes'],['fine','Fine'],['tedious','Tedious'],['draining','Drains']];

  const firstName = sourceActivity.participantName.split(' ')[0];
      ...
      <div style={{
        ...
        <div style={{ padding: '16px 22px', borderBottom: '1px solid var(--border-soft)', display: 'flex', alignItems: 'center', gap: 14 }}>
          <div style={{ flex: 1 }}>
            ...
            <h2 className="wa-display" style={{ fontSize: 19, margin: '4px 0 0', fontWeight: 500 }}>
              Select cards to merge — a new card is created, originals are kept.
            </h2>
          </div>
          <button onClick={onClose} style={{ background: 'transparent', border: 0, fontSize: 22, color: 'var(--muted)', cursor: 'pointer', padding: 4, lineHeight: 1 }}>×</button>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', flex: 1, overflow: 'hidden' }}>
          ...
          <div style={{ borderRight: '1px solid var(--border-soft)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            ...
            <div style={{ padding: '14px 18px 10px', borderBottom: '1px solid var(--border-soft)', flexShrink: 0 }}>
              <Eyebrow>Select cards to merge · {nSelected} selected</Eyebrow>
            </div>
            ...
            <div style={{ padding: '10px 18px', borderBottom: '1px solid var(--border-soft)', background: 'var(--paper)', flexShrink: 0 }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                {/* Pre-checked dark checkbox */}
                <span style={{
                  width: 16, height: 16, borderRadius: 3, background: 'var(--ink)', border: '1px solid var(--ink)',
                  display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
                  color: 'var(--cream)', fontSize: 10, flexShrink: 0, marginTop: 2,
                }}>✓</span>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 3 }}>
                    <span className="wa-avatar is-sm" style={{ background: sourceActivity.participantColor, color: '#fff' }}>{sourceActivity.participantInitials}</span>
                    <span style={{ fontSize: 12.5, fontWeight: 500, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{sourceActivity.title}</span>
                  </div>
                  <div className="wa-mono" style={{ fontSize: 10.5, color: 'var(--muted)' }}>
                    {firstName} · {sourceActivity.tpo} · {sourceActivity.freq}
                  </div>
                </div>
                <span className="wa-chip is-ghost" style={{ fontSize: 10, padding: '2px 6px', flexShrink: 0 }}>starting card</span>
              </div>
            </div>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '8px 0' }}>
              {filtered.length === 0 && (
                <div style={{ padding: '16px 18px', fontSize: 12, color: 'var(--muted-2)', fontStyle: 'italic' }}>No matching activities.</div>
              )}
              {filtered.map(({ a, score }) => {
                const isSelected = selectedIds.includes(a.id);
                const showBadge = score > 0.15;
                const badgeClass = score > 0.6 ? 'is-rust' : score > 0.3 ? 'is-amber' : '';
                const aFirstName = a.participantName.split(' ')[0];
                return (
                  <div
                    key={a.id}
                    onClick={() => onToggleId(a.id)}
                    style={{
                      padding: '9px 18px', cursor: 'pointer', display: 'flex', alignItems: 'flex-start', gap: 10,
                  ...
                  >
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 3 }}>
                        <span className="wa-avatar is-sm" style={{ background: a.participantColor, color: '#fff' }}>{a.participantInitials}</span>
                        <span style={{ fontSize: 12.5, fontWeight: 500, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{a.title}</span>
                      </div>
                      <div className="wa-mono" style={{ fontSize: 10.5, color: 'var(--muted)' }}>
                        {aFirstName} · {a.tpo} · {a.freq}
                      </div>
                    </div>
                    {showBadge && (
                      <span className={`wa-chip ${badgeClass}`} style={{ fontSize: 10, padding: '2px 6px', flexShrink: 0 }}>
          ...
          <div style={{ overflowY: 'auto', padding: '18px 22px', display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div>
              <Eyebrow>Merged card preview</Eyebrow>
              <p style={{ fontSize: 12, color: 'var(--muted)', margin: '4px 0 0', lineHeight: 1.5 }}>
                Edit the merged card before creating it. Originals will be kept but hidden from facilitator views.
              </p>
            </div>
                ...
                {TPO_OPTS.map(([v, lbl]) => {
                  ...
                  return (
                    <button key={v} onClick={() => onTpo(v)} style={{
                      ...
                      borderRadius: 4, cursor: 'pointer', fontWeight: on ? 500 : 400,
                    }}>{lbl}</button>
                  );
                })}
              </div>
            ...
            {/* Preview chip summary */}
            <div style={{ background: 'var(--paper)', border: '1px solid var(--border-soft)', borderRadius: 5, padding: '12px 14px' }}>
              <div className="wa-mono" style={{ fontSize: 10, color: 'var(--muted-2)', marginBottom: 8 }}>↳ PREVIEW</div>
              <div style={{ fontSize: 13.5, fontWeight: 500, marginBottom: 8 }}>{title || '(no title)'}</div>
        ...
        <div style={{ padding: '14px 22px', borderTop: '1px solid var(--border-soft)', display: 'flex', alignItems: 'center', gap: 12 }}>
          <span style={{ fontSize: 12.5, color: canMerge ? 'var(--ink-2)' : 'var(--muted)', flex: 1 }}>
            {canMerge
              ? `Creating 1 merged card from ${nSelected} originals.`
              : 'Select at least one more card to merge.'}
          </span>
          <button className="wa-btn is-ghost" style={{ padding: '8px 14px', fontSize: 13 }} onClick={onClose}>Cancel</button>
          <button
            className="wa-btn is-rust"
            style={{ padding: '8px 16px', fontSize: 13, opacity: canMerge ? 1 : 0.4, cursor: canMerge ? 'pointer' : 'not-allowed' }}
            onClick={onMerge}
            disabled={!canMerge}
          >
            ⇄ Create merged card →
