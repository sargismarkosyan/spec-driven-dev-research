  ...
  return (
    <div className="wa-layout">
      ...
      <div style={{ flex: 1, overflow: 'hidden', position: 'relative' }}>
        {view === 'live'    && <LiveView activities={activities} participants={participants} perPerson={perPerson} onEdit={openEdit} onRemove={handleRemove} />}
...
function LiveCard({ a, onEdit, onRemove }: {
  a: Activity;
  ...
  return (
    <div className="wa-activity">
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
        ...
        <span style={{ marginLeft: 'auto' }}><FacActions size="sm" onEdit={() => onEdit(a)} onRemove={() => onRemove(a.id)} /></span>
      </div>
...
function LiveView({ activities, participants, perPerson, onEdit, onRemove }: {
  activities: Activity[];
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '260px 1fr 1fr', height: '100%' }}>
      ...
      <main style={{ padding: '24px 28px', overflowY: 'auto', background: 'var(--cream)', borderRight: '1px solid var(--rule)' }}>
        ...
        {recent.length === 0 ? (
          ...
          <div style={{ display: 'grid', gap: 8 }}>
            {recent.map(a => (
              <LiveCard key={a.id} a={a} onEdit={onEdit} onRemove={onRemove} />
            ))}
          </div>
        )}
      </main>
